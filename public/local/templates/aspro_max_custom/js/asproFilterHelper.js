function asproFilterHelper(smartFilter, noAjax) {
	...
	if(this.resultDiv.length) {
		...
		$(document).on('click', '#filter-helper .filterHelperItem.multiple .title-wrapper > div', function(e) {
			var _el = $(e.target);
			var id = _el.closest('.select-value').data('id');
			_this.clearValue({ID: id});
		});
...
asproFilterHelper.prototype.show = function() {
	...
	if(this.resultDiv.length && this.filterHelper && Object.keys(this.filterHelper).length) {
		...
		this.resultDiv.append('<button type="reset" name="del_filter" id="del_filter">Очистить фильтр</button>')
...
asproFilterHelper.prototype.getResultHTML = function(el) {
	...
	var closeSvg = '';
	closeSvg += '<svg width="10" height="11" viewBox="0 0 10 11" fill="none" xmlns="http://www.w3.org/2000/svg">';
		closeSvg += '<path d="M1 1L9 10M9 1L1 10" stroke="white" stroke-width="2" stroke-linecap="round"/>';
	closeSvg += '</svg>';

	var resultText = '';
	resultText += '<div class="filterHelperItem colored_theme_bg rounded3'+(options.ITEM_CLASS === undefined ? '' : ' '+options.ITEM_CLASS)+'" data-id="'+(!options.VALUES || options.TYPE == 'price' ? options['NAME'] : '')+'">';

		resultText += '<div class="title-wrapper">';
			if (!options.VALUES || options.TYPE == 'price') {
				if(options['TITLE']) {
				...
				resultText += '<span class="delete_filter">';
					resultText += closeSvg;
				resultText += '</span>';
			} else {
				options.VALUES.forEach(function(el, i) {
					...
					resultText += '<div class="options-value">'
					resultText += '<span class="value select-value" data-id="'+el['NAME']+'">';
					resultText += el.VALUE;
					resultText += '</span>';
					resultText += '<span class="delete_filter">';
					resultText += closeSvg;
