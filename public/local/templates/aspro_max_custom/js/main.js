...
if(!funcDefined("setViewedProduct")){
	function setViewedProduct(id, arData){
		//ecommerce событие просмотра элемента каталога
		...
		dataLayer.push({
			...
			ecommerce: {
				...
				items: [{
					...
					"item_id": id,
					"price": arData.MIN_PRICE.ROUND_VALUE_VAT,
					"index": 1,
					"quantity": 1
				}]
		}
	...
	function initSelects(target){
		...
		$(target).find('.wrapper1 select:visible').ikSelect({
			...
			onShow: function(inst){
				...
				if(client_height<224){
					inst.$listInner.css('height', 'auto');
				}else{
					inst.$listInner.css('height', '224px');
				}
...
CheckTabActive = function(){
	if(typeof(clicked_tab) && clicked_tab)
	{
		...
		{
			clicked_tab--;
			$('.nav.nav-tabs li:not(.hidden)').each(function(){
...
InitFlexSliderByClass = function($class) {
	if($class.hasClass('flexslider-init') || $class.hasClass('flexslider-custom'))
		return;
	var slider = $class;
	var options;
	var defaults = {
...
$(document).ready(function(){
	...
	if(arAsproOptions['THEME']['PHONE_MASK']){
		...
		let mask = arAsproOptions['THEME']['PHONE_MASK'];
		$('input.phone').inputmask('mask', {'mask': mask});
	}
	...
	$(document).on('mouseenter', '.section-gallery-wrapper .section-gallery-wrapper__item', function(){
		let parent = $(this).parents('.thumb');
		$(this).siblings().removeClass('_active');
		$(this).addClass('_active');
		parent.find('.slide_separators span').removeClass('_active');
		parent.find('.slide_separators span').eq($(this).index()).addClass('_active');
	});
			...
			if($(this).parent().data("offers")!="Y"){
				...
				if(parseFloat($(this).parents(".counter_block").find(".plus").data('max'))>0){
					...
					if(curValue > ratio){
						...
						if($(input).parents('.expandables-block').is('[data-block="expandables"]') && curValue < 0){
							input.val(0);
						}
						else if($(input).parents('.expandables-block').is('[data-block="expandables"]') && curValue == 0){
							input.val(curValue);
						}
						else if(ratio){
							input.val(ratio);
	...
	$(document).on("change", ".counter_block input[type=text]", function(e){
		if(!$(this).parents('.basket_wrapp').length){
			...
			var plusMax = parseFloat($(this).parents(".counter_block").find(".plus").data('max'));
			if(plusMax>0 && !isNaN(plusMax)){
				if(val>plusMax){
					val=plusMax;
					// val -= (val % ratio);
				}
			...
			if($(this).parents('.expandables-block').is('[data-block="expandables"]')){
				//val = 0;
			}
			else if(val<ratio){
				val=ratio;
	...
	$(document).on( 'click', '.to-cart:not(.read_more), .basket_item_add', function(e){
		...
		if(!th.hasClass('clicked'))
		{
			...
			fill_prop.item=item;
			fill_prop.basket_props=basket_props;
			fill_prop.custom_price=$(this).attr('data-value');
			fill_prop.custom_full_price=$(this).attr('data-full-price');
			fill_prop.custom_currency=$(this).attr('data-currency');
			...
			if( !bIsService && (isDetail || isSku2) ){
				...
				allServicesOn.each(function(){
					...
					servicesNeedAdd[buttonCart.data('item')]['quantity'] =  buttonCart.attr('data-quantity');
					servicesNeedAdd[buttonCart.data('item')]['iblock_id'] = buttonCart.data('iblockid');
					servicesNeedAdd[buttonCart.data('item')]['custom_price'] = _this.closest('.services-item').data('price');
				});
				servicesNeedAdd = array_values_js(servicesNeedAdd);
				fill_prop.services = servicesNeedAdd;
				bIsService = true;
			...
			if($(th).parents('.catalog_detail').find('.expandables-block').is('[data-block="expandables"]')){
				var assocNeedAdd = [];
				$('.expandables-block .catalog_item').each(function(index, el) {
					var _this = $(this);
					//var buttonCart = _this.closest('.services-item').find('.to-cart');
					var assocItemQuantity = _this.find('.counter_block input[name="quantity"]').val();
					var assocItemPrice = parseInt(_this.find('.price_value').html());
					if(assocItemPrice > 0 && assocItemQuantity > 0){
						assocNeedAdd[_this.data('item')] = {};
						assocNeedAdd[_this.data('item')]['id'] = _this.data('item');
						assocNeedAdd[_this.data('item')]['iblock_id'] = _this.data('iblockid');
						assocNeedAdd[_this.data('item')]['quantity'] =  assocItemQuantity;
					}
					//assocNeedAdd[buttonCart.data('item')]['custom_price'] = _this.closest('.services-item').data('price');
				});
				assocNeedAdd = array_values_js(assocNeedAdd);
				fill_prop.assocNeedAdd = assocNeedAdd;
			}

			if($(th).parents('.catalog_detail').find('.equipment-block').is('[data-block="equipment"]')){
				var equipmenNeedAdd = [];
				$('.equipment-block li.active').each(function(index, el) {
					var _this = $(this);
					//var buttonCart = _this.closest('.services-item').find('.to-cart');
					var equipmenItemQuantity = 1;
					var equipmenItemPrice = parseInt(_this.attr('data-price'));
					if(equipmenItemPrice > 0 && equipmenItemQuantity > 0){
						equipmenNeedAdd[_this.data('item')] = {};
						equipmenNeedAdd[_this.data('item')]['id'] = _this.data('item');
						equipmenNeedAdd[_this.data('item')]['iblock_id'] = _this.data('iblockid');
						equipmenNeedAdd[_this.data('item')]['quantity'] =  equipmenItemQuantity;
						equipmenNeedAdd[_this.data('item')]['custom_price'] = equipmenItemPrice;
					}
				});
				equipmenNeedAdd = array_values_js(equipmenNeedAdd);
				fill_prop.services = fill_prop.services.concat(equipmenNeedAdd);
			}

			var servicesWrap = th.closest('.product-container').find('.buy_services_wrap');
			if( bIsService ){
			...
			if(th.data("empty_props")=="N"/* && fill_prop.part_props != 'Y'*/){
				...
				console.log('click basket_item_add');
				$.ajax({
					type:"POST",
					url:arAsproOptions['SITE_DIR']+"ajax/item.php",
					data:fill_prop,
					...
					success:function(data){
						...
						if($('.custom_service_chekbox .switch_block').hasClass('active')){
							$('.custom_service_chekbox.basket-item-property-custom').each(function(index, el) {
								if ($(el).is(':visible') && $(el).find('.switch_block.active').length) {
									let idProduct = $(this).data('item');
									let itemData = [];
									let idAcModule = $('.equipment-block-template li.active').attr('data-product');
									itemData.PRODUCT_ID = $('body .product-action .buy_services_wrap').attr('data-parent_product');//$('.equipment-block-template li.active').attr('data-item');
									itemData.OFFER = $('body .product-action .buy_services_wrap').attr('data-parent_product');
									itemData.IBLOCK_SERVICE_ID = $(this).data('iblockid');
									itemData.QUANTITY = $(this).data('quantity');
									let dataResult = {
										parentId : itemData.PRODUCT_ID,
										iblockid: itemData.IBLOCK_SERVICE_ID,
										quantity: itemData.QUANTITY,
										module: true,
										propOffer: itemData.OFFER,
									};
									if ($('.product' + idAcModule + ' .custom_service_chekbox' ).attr('data-price').length > 0) {
										itemData.PRICE = $(el ).attr('data-price');//Cборка аксессуара
										dataResult.priceModule = itemData.PRICE
									}
									
									console.log('main.js :', itemData);
									...
									$.ajax({
										...
										success: function(html) {
											...
											setTimeout(()=>{
												getActualBasketInAssembly(null, null, null, false, idProduct, itemData.PRODUCT_ID);
											},2500)
										}
									});
								}
	...
	$('.accordion-active-link').on('click', function(e){
        e.stopPropagation()
	});
...
if(!funcDefined('getActualBasketInAssembly')){
	getActualBasketInAssembly=function(iblockID, type, sync, $isServicesAssembly = false, $itemProductID = '', $itemDataID = ''){
		var data='';
		if(typeof iblockID !=="undefined" && iblockID){
			data={"iblockID":iblockID}
		}
		...
		if ($isServicesAssembly) {
			$('.basket-items-list-item-container[data-offer="' + $itemDataID + '"] .custom_service_chekbox[data-id="' + $itemProductID + '"] input[name=buy_switch_services]').prop('checked', false);
		} else {
			$('.basket-items-list-item-container[data-offer="' + $itemDataID + '"] .custom_service_chekbox[data-id="' + $itemProductID + '"] input[name=buy_switch_services]').prop('checked', true);
		}
		$('.basket-items-list-item-container[data-offer="' + $itemDataID + '"] .custom_service_chekbox[data-id="' + $itemProductID + '"]').css('pointer-events','');
	}
}

if(!funcDefined('getActualBasketInAssemblyModule')){
	getActualBasketInAssemblyModule=function(iblockID, type, sync, $isServicesAssembly = false, $itemProductID = '', $itemDataID = ''){
		var data='';
		if(typeof iblockID !=="undefined" && iblockID){
			data={"iblockID":iblockID}
		}
		...
		if ($isServicesAssembly) {
			$('.basket-items-list-item-container[data-offer="' + $itemDataID + '"] .services-item-module .custom_service_chekbox[data-id="' + $itemProductID + '"] input[name=buy_switch_services]').prop('checked', false);
		} else {
			$('.basket-items-list-item-container[data-offer="' + $itemDataID + '"] .services-item-module .custom_service_chekbox[data-id="' + $itemProductID + '"] input[name=buy_switch_services]').prop('checked', true);
		}
		$('.basket-items-list-item-container[data-offer="' + $itemDataID + '"] .services-item-module .custom_service_chekbox[data-id="' + $itemProductID + '"]').css('pointer-events','');
	}
}
				...
				if(typeof (BX.Sale.OrderAjaxComponent) == "object"){
					...
					if($('.bx-soa-cart-total-line-total').length && (asproShowLicence ||  asproShowOffer)){
						...
						if( (!$('.licence_block.filter').length && asproShowLicence) || (!$('.offer_block.filter').length && asproShowOffer) ){
							...
							$(document).on('click', '.lic_condition a', function(e){
...
function changeAssocSumm(){
	var assocSumm = 0;
	$('.expandables-block .catalog_item').each(function(index, el) {
		var assocItemQuantity = $(el).find('.counter_block input[name="quantity"]').val();
		var assocItemPrice = $(el).find('.price_value').text().replace(/\s+/g, '');
		assocSumm += assocItemPrice * assocItemQuantity;
	});
	if(assocSumm != 0){
		$('.expandables-block-summ-cost').removeClass('hidden');
		$('.expandables-block-summ-cost span').html('+ ' + assocSumm + ' ₽');

		if(!$('.services-block .js-services div').is('.expandables-block-summ-cost')){
			$('.services-block .js-services').prepend($('.expandables-block .expandables-block-summ-cost').clone());
			$('.services-block .js-services').addClass('active');
		}
	}
	else{
		//$('.expandables-block-summ-cost').addClass('hidden');
		$('.expandables-block-summ-cost span').html('+ 0 ₽');
		$('.services-block .js-services .expandables-block-summ-cost').remove();
	}
}

$(document).ready(function() {
	if($.cookie('noShowWarningMessage') !== 'Y') {
		if($("#warning-modal").length) {
			document.body.classList.add("lock");
			document.querySelector("#warning-modal").classList.add("warning-modal--open");
			$(document).on('click', '#warning-btn_close', function (e) {
				e.preventDefault();
				document.body.classList.remove("lock");
				document.querySelector("#warning-modal").classList.remove("warning-modal--open");
				var date = new Date();
				date.setTime(date.getTime() + (120 * 60 * 1000)); //Устанавливаем куки на 2 часа
				$.cookie('noShowWarningMessage', 'Y',{ expires: date });
			});
		}
	}


  let accordionsVacancies = $('.vacancy-item');
  $('.custom__dropdown').click(function() {
    $(this).attr('tabindex', 1).focus();
    $(this).toggleClass('active');
    $(this).find('.custom__dropdown-list').slideToggle(300);
  });
  $('.custom__dropdown').focusout(function() {
    $(this).removeClass('active');
    $(this).find('.custom__dropdown-list').slideUp(300);
  });
  $('.custom__dropdown .custom__dropdown-list li').click(function () {
    let currentId = $(this).attr('data-id');
    $(this).parents('.custom__dropdown').find('span').text($(this).text());

    $('.custom__dropdown .custom__dropdown-list li').each(function () {
    ...
    $(`.vacancy-item[data-id=${currentId}]`).each(function () {
      $(this).show();
      InitStickySideBar();
    })
  });
