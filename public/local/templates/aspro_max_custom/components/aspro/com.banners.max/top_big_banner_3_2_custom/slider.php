...
<div class="top_slider_wrapp maxwidth-banner<?=($bHideOnNarrow ? ' hidden_narrow' : '')?>">
	...
	<div class="flexslider hovers flexslider__custom">
		<ul class="slides">
...
/* 						echo "<!--------zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz";
					...
					<img class="box__img" alt="" src="<?=$background?>">
					<img class="box__img_mobile" alt="" src="<?=$mobileImg?>">
					
					<?if($arItem["PROPERTIES"]["URL_STRING"]["VALUE"]):?>
						<a class="target" href="<?=$arItem["PROPERTIES"]["URL_STRING"]["VALUE"]?><?= (strpos($arItem["PROPERTIES"]["URL_STRING"]["VALUE"], '?') != false) ?  '&' : '?'?><?= ($sCodeUTMBanners) ?  $sCodeUTMBanners .'=' .$arItem['ID'] : ''?>" <?=(strlen($target) ? 'target="'.$target.'"' : '')?>></a>
					<?endif;?>
