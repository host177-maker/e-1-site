				...
				if(bSend){
					...
					$.ajax({
						...
						success: function(data) {
							...
							// ГдеСлон
                            let productPriceSum = data.productSumm;

                            let slonItemCommission = {
                                id: '',
                                quantity: 1,
                            ...
                            data.arSlonItems.arrSlonItems.push(slonItemCommission);
							window.gdeslon_q = window.gdeslon_q || [];
							window.gdeslon_q.push({
								page_type: "thanks",
								merchant_id: "100062",
								order_id: data.orderId,//id заказа
								products: data.arSlonItems.arrSlonItems,
								deduplication: utm,
								<?if(!empty($GLOBALS['USER']->GetID())):?>user_id: "<?=$GLOBALS['USER']->GetID();?>"<?endif;?>
							});
							console.log('ГдеСлон popup end');//служит для проверки

							//getblue успешная транзакция купить в 1 клик
							window.blue_q = window.blue_q || [];
							window.blue_q.push(
								{event: "setCampaignId", value: "00817ED8-9D92-C9B7-14D7CD0015C22D53"}
								,{event: "setProductId", value: data.getblueIds} // id товаров
								,{event: "setTransactionTotal", value: data.productSumm} // сумма продуктов
								,{event: "setTransactionId", value: data.message} //id заказа
								,{event: "setPageType", value: "conversion"}
							);
							console.log('getblue popup end'); //служит для проверки
