        ...
        <form method="post" id="one_click_buy_form" action="<?= $arResult['SCRIPT_PATH'] ?>/script.php">
                ...
                <div class="form-control bg">
                    ...
                    <? else: ?>
                        <? if ($arResult["PROPS"][$field]["TYPE"] == "FILE"): ?>
                            <div class="files" data-code="<?= $field ?>"
                                data-required="<?= (in_array($field, $arParams['REQUIRED']) ? 'Y' : 'N'); ?>">
                                <div class="inner_file">
                                    <div class="wrapper_file">
                                        <span class="remove" title="<?= GetMessage("REMOVE_FILE"); ?>"><i></i></span>
                                        <input type="file"
                                            <? if (in_array($field, $arParams['REQUIRED'])): ?>required<? endif;
                                                                                                        ?> name="ONE_CLICK_BUY[<?= $field ?>][]">
                                    </div>
                                </div>
                            </div>
                            <? if ($arResult["PROPS"][$field]["MULTIPLE"] == "Y"): ?>
                                <div class="btn_block_file"><span
                                        class="btn btn-default btn-xs"><?= GetMessage("ADD_BTN"); ?></span></div>
                            <? endif; ?>
                        <? else: ?>
            ...
            <? if ($arParams["SHOW_LICENCE"] == "Y"): ?>
                <div class="form license_form <?= ($arParams['SHOW_OFFER'] == 'Y' ? 'w_offer_pub ' : '') ?>">
                    <div class="licence_block filter label_block">
                        <input type="checkbox"
                            id="licenses_popup_OCB" <?= (COption::GetOptionString("aspro.max", "LICENCE_CHECKED", "N") == "Y" ? "checked" : ""); ?>
                            name="licenses_popup_OCB" required value="Y">
            ...
            <? if ($arParams["SHOW_OFFER"] == "Y"): ?>
                <div class="form offer_pub_form">
                    <div class="offer_block filter label_block">
                        <input type="checkbox"
                            id="offer_popup_OCB" <?= (COption::GetOptionString("aspro.max", "OFFER_CHECKED", "N") == "Y" ? "checked" : ""); ?>
                            name="offer_popup_OCB" required value="Y">
            ...
            <? if ($arParams['SHOW_DELIVERY_NOTE'] === 'Y'): ?>
                <div class="delivery_note_wrapper">
                    ...
                    <div class="text"
                        style="display: none;"><? $APPLICATION->IncludeFile(SITE_DIR . "include/oneclick_delivery_text.php", array(), array("MODE" => "html")); ?></div>
                </div>
            <? endif; ?>

            <div class="but-r clearfix">
                <!--noindex-->
                <button class="btn btn-default animate-load" type="submit" id="one_click_buy_form_button"
                    name="one_click_buy_form_button" value="<?= GetMessage('ORDER_BUTTON_CAPTION') ?>">
                    <span><?= GetMessage("ORDER_BUTTON_CAPTION") ?></span></button>
                <!--/noindex-->
            </div>
...
<script type="text/javascript">
    ...
    if (!funcDefined('showOneClickSmsCode')) {
        function showOneClickSmsCode(data) {
            ...
            new BX.PhoneAuth({
                ...
                data: {
                    signedData: data.ext.SIGNED_DATA
                },
            ...
            if (data.result == 'Y') {
                if (typeof data.ext === 'object' && data.ext.CODE === 'SHOW_SMS_FIELD') {
                    ...
                    if (arMaxOptions['COUNTERS'][(type == 'basket' ? 'USE_FASTORDER_GOALS' : 'USE_1CLICK_GOALS')] !== 'N') {
                        var eventdata = {
                            goal: (type == 'basket' ? 'goal_fastorder_success' : 'goal_1click_success')
                        };
                    ...
                    if (bSend) {
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                // console.log('16_1_order_shk_step_1_complite');

                                // ГдеСлон
                                var productPriceSum = data.productSumm;
                                ...
                                data.arrSlonItems.push(slonItemCommission);
                                ...
                                dataLayer.push({
                                    ...
                                    'eventAction': '16_1_order_shk_step_1_complite',
                                    'eventLabel': '',
                                    'price': data.productSumm
                                });
                                console.log('FB успешная транзакция купить в 1 клик');
                                // console.log(data);
                                /*fbq('track', 'Purchase', {
                                    value: data.productSumm,
                                    currency: 'RUB',
                                    contents: data.fbData,
                                });*/
                                //mail.ru успешная транзакция купить в 1 клик
                                var _tmr = _tmr || [];
                                _tmr.push({
                                    id: '2828139',
                                    type: 'itemView',
                                    ...
                                    pagetype: 'purchase',
                                    list: '1',
                                    totalvalue: data.productSumm
                                });
                                //mgid успешная транзакция купить в 1 клик
                                /*(function() {
                                ...
                                //getblue успешная транзакция купить в 1 клик
                                ...
                                // var comagicSendData = data.arComagicSendData;
                                // Comagic.addOfflineRequest(comagicSendData);
                                // console.log('comagicSendData');
                                // console.log(comagicSendData);

                                console.log('VK успешная транзакция купить в 1 клик');
                                //VK успешная транзакция купить в 1 клик
                                //window.vkAsyncInit = function() {
                                VK.Retargeting.Init('VK-RTRG-333568-aT9Ur');
                                const eventParams = {
                                    "products": data.vkData,
                                    "total_price": data.productSumm
                                };
                                // console.log(eventParams);
                                VK.Retargeting.ProductEvent(2842, "purchase", eventParams);
                                //}
                                ...
                                dataLayer.push({
                                    event: "purchase",
                                    ecommerce: {
                                        value: <?= $arResult['ECOM_DATA']['price'] ?>,
                                        transaction_id: data.message,
                                        currency: "RUB",
                                        items: <?= \CUtil::PHPToJSObject($arResult['ECOM_DATA']['items']) ?>
                                    }
                                });

                                ecommerceOnFormSubmit('buy_in_one_click');
    ...
    $(document).ready(function() {
        ...
        <? else: ?>
            if (arMaxOptions['COUNTERS']['USE_1CLICK_GOALS'] !== 'N') {
                var eventdata = {
                    goal: 'goal_1click_begin'
                };
                BX.onCustomEvent('onCounterGoals', [eventdata])
            }
        <? endif; ?>

        $('form#one_click_buy_form input[name="PAGE_URL"]').attr('value', window.location.href);
    });
    ...
    <? if (!$arParams['INLINE_FORM']): ?>
        if (arMaxOptions['THEME']['PHONE_MASK']) {
            $('#one_click_buy_id_PHONE').inputmask("mask", {
                "mask": arMaxOptions['THEME']['PHONE_MASK'],
