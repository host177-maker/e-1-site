...
<?if($arResult["ITEMS"]):?>
	...
	?>
	<p class="product-param-choose-title expandables-block-title"><span><?=$iScuBlockCounter;?>. </span>Аксессуары</p>

	<div class="expandables-block-summ-cost"><span>+ 0 ₽</span>будет добавлено в корзину</div>
	...
	<div class="top_wrapper items_wrapper <?=$templateName;?>_template <?=$arParams['IS_COMPACT_SLIDER'] ? 'compact-catalog-slider' : ''?>">
		...
		<div class="catalog_block items row <?/*=$arParams['IS_COMPACT_SLIDER'] ? 'swipeignore mobile-overflow' : ''?> margin0 <?=$bHasBottomPager ? 'has-bottom-nav' : ''*/?> js_append ajax_load block flexbox<?=($bSlide ? ' owl-carousel owl-theme owl-bg-nav visible-nav short-nav hidden-dots swipeignore ' : '');?>"<?if($bSlide):?>data-plugin-options='{"nav": true, "autoplay" : false,  "autoplayTimeout" : "3000", "smartSpeed":1000, <?=(count($arResult["ITEMS"]) > 4 ? "\"loop\": true," : "")?> "responsiveClass": true, "responsive":{"0":{"items": 2},"600":{"items": 2},"768":{"items": 3},"1200":{"items": 3}}}'<?endif;?>>
	<?endif;?>
		<?
		...
		<?foreach($arResult["ITEMS"] as $arItem){?>
			...
			<?ob_start();?>
				<div class="counter_wrapp big clearfix">
					<?//if(($arAddToBasketData["OPTIONS"]["USE_PRODUCT_QUANTITY_DETAIL"] && $arAddToBasketData["ACTION"] == "ADD") && $arAddToBasketData["CAN_BUY"]):?>
						<?
							$arAddToBasketData['OPTIONS']['USE_PRODUCT_QUANTITY_DETAIL'] = 'Y';
							$arAddToBasketData["MIN_QUANTITY_BUY"] = 0;
							//$arAddToBasketData['OPTIONS']['MAX_QUANTITY_BUY'] = 100;
						?>
						<?=\Aspro\Functions\CAsproMax::showItemCounter($arAddToBasketData, $arItem["ID"], $arItemIDs, $arParams, 'md', '', true, true);?>
					<?//endif;?>
				</div>
			<?$counterBlock = ob_get_clean();?>
			...
			<div class="col-lg-<?=$col;?><?if($bBigBlock):?> col-md-8 col-sm-12 col-xs-12<?else:?> col-md-4 col-sm-6 col-xs-12<?endif;?> col-xxs-12 item item-parent item_block<?=($bBigBlock ? ' big' : '');?> <?=($arParams['SET_LINE_ELEMENT_COUNT'] ? 'custom-line' : '');?>">
				...
				<div class="catalog_item_wrapp catalog_item item_wrap main_item_wrapper<?=($bBigBlock ? ' big' : '');?> <?=($bFonImg ? '' : ' product_image')?> <?=($arItem["OFFERS"] ? 'has-sku' : '')?>" id="<?=$arItem["strMainID"]?>" data-item="<?=$arItem["ID"]?>" data-iblockid="<?=$arItem["IBLOCK_ID"]?>">
	...
	<?if($arParams["AJAX_REQUEST"] != "Y"):?>
		...
		<?if(count($arResult["ITEMS"]) > 3):?>
			<div class="catalog_assoc_show_all"><p><span><?=GetMessage('CATALOG_ASSOC_SHOW_ALL');?></span><i class="fa fa-angle-down" aria-hidden="true"></i></p></div>
		<?endif;?>
	</div>
	<?endif;?>
