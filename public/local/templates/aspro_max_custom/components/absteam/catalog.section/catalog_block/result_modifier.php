...
if (!empty($arResult['ITEMS'])) {
    ...
    foreach ($arResult['ITEMS'] as $key => $arItem) {
        ...
        if ($arItem['CATALOG'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS'])) {
            ...
            foreach ($arSKUPropIDs as $propkey => $strOneCode) {
                ...
                foreach ($arMatrix as $keyOffer => $arRow) {
                    if ($boolExist) {
                        ...
                        if ($strOneCode == 'COLOR_REF') {
                            $arPropSKU[$strOneCode] = $arSKUPropList[$strOneCode]["VALUES"];
                        } else {
            ...
            if (!$arItem['OFFERS_PROP']) {
                ...
                $arItem['OFFERS_SELECTED'] = 0; //fix select sku image in list
        ...
        $bNeedFindPicture = ($arParams['SHOW_GALLERY'] == 'Y') && (CMax::GetFrontParametrValue("SHOW_FIRST_SKU_PICTURE") == "Y");
    ...
    $arResult['ECOM_DATA'] = [];
    $listName = $arResult['NAME'];
    //немного костыльное решение на определение страницы поиска
    if (isset($arResult['ORIGINAL_PARAMETERS']['GLOBAL_FILTER']['=ID'])) {
        $listName = 'Результаты поиска';
    }
    if (isset($arResult['ITEMS']) && !empty($arResult['ITEMS'])) {
        ...
        foreach ($arResult['ITEMS'] as $index => &$arItem) {
            ...
            $productArrBySlon[] = array('id' => $arItem['ID'], 'price' => $arItem['MIN_PRICE']['DISCOUNT_VALUE'] ? $arItem['MIN_PRICE']['DISCOUNT_VALUE'] : $arItem['MIN_PRICE']['VALUE'], 'quantity' => 1);
        }
        if (!empty($arEcomData)) {
