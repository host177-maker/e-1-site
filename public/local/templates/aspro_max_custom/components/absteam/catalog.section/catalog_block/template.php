...
<? if ($arResult["ITEMS"]) : ?>
			...
			<? foreach ($arResult["ITEMS"] as $arItem) { ?>
				...
				if ($bUseSkuProps) {
					if (!$arItem["OFFERS"]) {
						...
						$arParams['RESTRICTION'] = $arItem["RESTRICTION"];
						...
						if ($arParams['REVIEWS_VIEW']) : ?>
							<? $APPLICATION->IncludeComponent(
								...
								array("HIDE_ICONS" => "Y")
									...
									//передаем переменную с рейтингом
									global $intRationValue; ?>
                        <?
                        if ($arItem['PROPERTIES']['RATING']['VALUE']) {
                            $intRationValue = (int)$arItem['PROPERTIES']['RATING']['VALUE'];
                            // Ensure value is between 0 and 5
                            $intRationValue = max(0, min(5, $intRationValue));
                        }
                        ?>
							<div class="blog-info__rating--top-info section_rating id<?= $arItem["ID"] ?>">
								...
								<span class="font_sxs rating_bold"><?= (!empty($intRationValue)) ? $intRationValue . '/5' : '' ?></span>
							</div>
							<script>
								var rating_value = <?= CUtil::PhpToJSObject($intRationValue) ?>;
                ...
                <?php
                ob_start();
                include 'parts/split.php';
                $itemSaBlock = ob_get_clean();
                ?>

				<? ob_start(); ?>
				<div class="item-title">
					<a href="<?= $arItem["DETAIL_PAGE_URL"] . '?oid=' . $arItem["OFFERS"][$arItem["OFFERS_SELECTED"]]["ID"]; ?>" onclick="onProductClick(<?= \CUtil::PHPToJSObject($arItem['ECOM_ITEM_DATA']) ?>)" class="<?= $bBigBlock && $bFonImg ? '' : 'dark_link' ?> option-font-bold font_sm"><span><?= $elementName; ?></span></a>
				</div>
				<? if ($arItem["RESTRICTION"]) : ?>
					<div class="item-restriction">Покупка возможна только при условии приобретения шкафа.</div>
					<style>
						#<?= $arItem["strMainID"] ?>.wrapp_one_click {
							display: none !important;
						}

						#<?= $arItem["strMainID"] ?>.button_block {
							width: 100% !important;
						}
					</style>
					<script>
						<? $id =  $arItem['OFFERS'][0]['ID'] ?? $arItem['ID']; ?>
						BX.addCustomEvent('onCompleteAction', function(data, el) {

							if (data.action == 'loadForm' && $(el).hasClass('to-cart') && $(el).data('item') == <?= $id ?>) {
								$.ajax({
									type: "POST",
				...
				<div class="cost prices clearfix">
					...
					<? if ($arItem["OFFERS"]) : ?>
						...
						<div class="js_price_wrapper">
							...
							<?php
							$widgetPrice = data_get($arItem, 'MIN_PRICE', []);
							?>
							<yandex-pay-badge
                                    merchant-id="5ef67561-d34e-45e2-bb25-8320a4653548"
                                    type="cashback"
                                    amount="<?= $widgetPrice["DISCOUNT_VALUE"] ?>"
                                    size="m"
                                    theme="light"
                                    align="left"
                                    color="primary"
						...
						<? if ($arCurrentSKU) : ?>
							...
							<div class="ce_cmp_visible">
								<div class="js_price_wrapper">
									<?
									$widgetPrice = array_values($arCurrentSKU["PRICES"]);
									$arParamsCE_CMP["SHOW_DISCOUNT_PERCENT"] = "N";
									...
									<yandex-pay-badge merchant-id="5ef67561-d34e-45e2-bb25-8320a4653548" type="cashback" amount="<?= $widgetPrice["DISCOUNT_VALUE"] ?>" size="m" theme="light" align="left" color="primary" />
								</div>
							</div>
						<? endif; ?>
					<? else : ?>
				...
				<? ob_start(); ?>
				<div class="footer_button <?= ($arItem["OFFERS"] && $arItem['OFFERS_PROP'] ? 'has_offer_prop' : ''); ?> <?= ($arParams["TYPE_VIEW_BASKET_BTN"] == "TYPE_2" ? ' n-btn' : '') ?>">
					<? if ($arParams["TYPE_VIEW_BASKET_BTN"] != "TYPE_2" || $bBigBlock) : ?>
								...
								<? } else { ?>
									<div class="offer_buy_block offer_buy_block_new">
										...
										<div class="like_blocks_new ce_cmp_hidden">
											<? \Aspro\Functions\CAsproMaxItem::showDelayCompareBtnForBlock($arParams, $arItem, $arAddToBasketData, $totalCount, $bUseSkuProps, 'block', false, false, '_small', $currentSKUID, $currentSKUIBlock); ?>
										</div>
									</div>
									<? if (isset($arCurrentSKU['PRICE_MATRIX']) && $arCurrentSKU['PRICE_MATRIX']) // USE_PRICE_COUNT
									{ ?>
				...
				<div class="inner_content js_offers__<?= $arItem['ID']; ?>_<?= $arParams["FILTER_HIT_PROP"] ?> col-lg-<?= $col; ?><? if ($bBigBlock) : ?> col-md-8 col-sm-12 col-xs-12<? else : ?> col-md-4 col-sm-6 col-xs-6<? endif; ?> col-xxs-12 item item-parent item_block<?= ($bBigBlock ? ' big' : ''); ?> <?= ($arParams['SET_LINE_ELEMENT_COUNT'] ? 'custom-line' : ''); ?> <?= (!empty($arItem["PROPERTIES"]["SHOW_ICON_SPECIAL"]["VALUE"]) ? ' special ' : ''); ?> <? if ($arItem["RESTRICTION"]) : ?>no_icon<? endif; ?>" data-id="<?= $arItem["ID"]; ?>">
					...
					<div
                        class="catalog_item_wrapp catalog_item item_wrap main_item_wrapper<?= ($bBigBlock ? ' big' : ''); ?> <?= ($bFonImg ? '' : ' product_image') ?> <?= ($arItem["OFFERS"] ? 'has-sku' : '') ?>"
                        id="<?= $arItem["strMainID"] ?>"
                    >
						<div class="inner_wrap <?= $arParams["TYPE_VIEW_BASKET_BTN"] ?>">
							...
							<? if ($bBigBlock) : ?>
								<? if ($bFonImg) : ?>
									<a class="absolute-full-block lazy absolute-full-block_bg_center darken-bg-animate" href="<?= $arItem["DETAIL_PAGE_URL"] . '?oid=' . $arItem["OFFERS"][$arItem["OFFERS_SELECTED"]]["ID"]; ?>" onclick="onProductClick(<?= \CUtil::PHPToJSObject($arItem['ECOM_ITEM_DATA']) ?>)" data-src="<?= $imageSrc ?>" data-bg="<?= $imageSrc ?>" style="background-image:url(<?= \Aspro\Functions\CAsproMax::showBlankImg($imageSrc); ?>)"><span></span></a>
								<? endif; ?>
							<? endif; ?>
							<div class="image_wrapper_block<?= ($arParams['SHOW_PROPS'] == 'Y' && $arItem['DISPLAY_PROPERTIES'] && !$bBigBlock ? ' with-props' : ''); ?>">
								...
								<? if (!$bBigBlock) : ?>
									...
									<?= $itemDiscountTime ?>
									<?php /*
									<? if (count($arItem['GALLERY']) > 0) : ?>
										<!-- СЛАЙДЕР НА КАРТОЧКЕ ТОВАРА -->
										<a href="<?=$arItem['DETAIL_PAGE_URL']?>" class="">
											<div class="images-slider js-images-slider">
												<div class="images-slider__dots js-images-slider__dots">
													<? foreach ($arItem['GALLERY'] as $key => $arImage) : ?>
														<div class="images-slider__dot js-images-slider__dot"></div>
													<? endforeach; ?>
												</div>
												<div class="images-slider__controls js-images-slider__controls">
													<? foreach ($arItem['GALLERY'] as $key => $arImage) : ?>
														<div
															data-src="<?= $arImage['SRC'] ?>"
															class="images-slider__control js-images-slider__control"></div>
														<?php
														// var_dump($arImage);
														?>
													<? endforeach; ?>
												...
												<img
													class="images-slider__image js-images-slider__image"
													src="<?= $arItem['GALLERY'][0]['SRC'] ?>"
													alt="" />
											</div>
										</a>
									<? endif; ?>
									*/?>
									...
									<?php
										$arItem['DETAIL_PAGE_URL']  = $arItem['DETAIL_PAGE_URL'] . '?oid=' . $arItem["OFFERS"][$arItem["OFFERS_SELECTED"]]["ID"];;
									?>
									<? if ($arParams['SHOW_GALLERY'] == 'Y' && !empty($arItem['GALLERY'])) : ?>
	...
	<? if ($arParams["AJAX_REQUEST"] == "Y") : ?>
		...
		<div class="bottom_button_wrapper">
			...
			<div class="form">
				<div class="form_description"><?=\GetMessage('SELECTION_WARDROBE_DDESCRIPTION')?></div>
				<div class="form-action">
					<div class="animate-load" data-event="jqm" data-param-form_id="SELECTION_WARDROBE" data-name="SELECTION_WARDROBE">
							<div class="more_text_ajax btn btn-transparent-border-color btn-sm has-ripple"><?=\GetMessage('SELECTION_ORDER_BUTTON')?></div>
					</div>
				</div>
			</div>
		</div>
...
<!-- <script src="<?//= SITE_TEMPLATE_PATH ?>/js/catalog-card-slider.js"></script> -->