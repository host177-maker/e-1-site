...
?>
<div class="maxwidth-theme1 franchise__form1">
<div class="form  <?=$arResult["arForm"]["SID"]?>">
	...
	<?if($arParams['HIDE_SUCCESS'] != 'Y' || ($arParams['HIDE_SUCCESS'] == 'Y' && !strlen($arResult["FORM_NOTE"]))):?>
		...
		<?=bitrix_sessid_post();?>
		<div class="form_body form-body-fr">
			<?if(is_array($arResult["QUESTIONS"])):?>
        <?if(!$bLeftAndRight):?>
        ...
        <?else:?>
					<div class="row">
						...
						<div class="col-md-6">
							<?foreach($arResult["QUESTIONS"] as $FIELD_SID => $arQuestion):?>
								<?if($arQuestion["STRUCTURE"][0]["FIELD_PARAM"] != 'left' && $arQuestion["STRUCTURE"][0]["FIELD_PARAM"] != 'full'):?>
									<?CMax::drawFormField($FIELD_SID, $arQuestion);?>
								<?endif;?>
							<?endforeach;?>
						</div>
		...
		<div class="form_footer form-form-fr">
			...
			<div class="form-send-wrapper">
				<button type="submit" class="btn btn-default has-ripple" value="submit" ><span><?=$arResult["arForm"]["BUTTON"]?></span></button>
				<input type="hidden" class="btn btn-default" value="<?=$arResult["arForm"]["BUTTON"]?>" name="web_form_submit">
			</div>
			<? if ($arResult["arForm"]["SID"] != 'FEEDBACK_VACANCY') {?>
