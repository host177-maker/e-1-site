...
$arSectionsFilter = array('IBLOCK_ID' => $catalog_id, 'ACTIVE' => 'Y', 'GLOBAL_ACTIVE' => 'Y', 'ACTIVE_DATE' => 'Y', 'UF_IS_TAG' => 0, '<DEPTH_LEVEL' => $arParams['MAX_LEVEL']);//убираем разделы, которые выводятся как тег UF_IS_TAG
...
if($arSections){
	...
	{
		$bSelect = true;
		foreach($arItem['CHILD'] as $Subkey => $arSubItem)
		{
			foreach($arSubItem['CHILD'] as $Sub2key => $arSub2Item)
			{
				if($arSub2Item['SELECTED'] && $bSelect){
					$arResult[$key]['CHILD'][$Subkey]['CHILD'][$Sub2key]['SECTION_PAGE_URL'] = '';
					$bSelect = false;
				}
			}
			if($arSubItem['SELECTED'] && $bSelect){
				$arResult[$key]['CHILD'][$Subkey]['SECTION_PAGE_URL'] = '';
				$bSelect = false;
			}
		}
		if($arItem['SELECTED'] && $bSelect){
			$arResult[$key]['SECTION_PAGE_URL'] = '';
		}
	}
}?>