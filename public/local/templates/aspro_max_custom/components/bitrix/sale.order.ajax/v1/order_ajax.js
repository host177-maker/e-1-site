	...
	BX.Sale.OrderAjaxComponent = {
		...
		refreshOrder: function(result)
		{
			...
			else
			{
				...
				//this.makeCustomHandler();

				BX.onCustomEvent(this, 'onRefreshOrder', [result]);
			}

			return true;
		},

		makeCustomHandler: function(){
			var propsIterator, property, obj = this;
			...
			while (property = propsIterator())
			{
				...
				console.log('makeCustomHandler');
				// console.log('property.getName()');
				// console.log(property.getName());
				// console.log(property.getType());
				console.log(property.getSettings());
				if(property.getSettings().IS_ADDRESS == 'Y'){
					//this.getPropertyRowNode(property, propsNodeCol, false);
					$('[name="ORDER_PROP_' + property.getSettings().ID + '"]').on("blur", function(){
						//obj.sendRequest();
						// setTimeout(function(){
		...
		createBasketItem: function(basketItemsNode, item, index, active)
		{
			var mainColumns = [],
				...
				i, tr, cols, dopColCount = 0, countCols = 0;
			...
			var withServices = typeof item.data['LINK_SERVICES'] !== 'undefined' && item.data['LINK_SERVICES'].constructor === Object;
			...
			basketItemsNode.appendChild(
				BX.create('TR', {
					props: {className: 'bx-soa-item-tr bx-soa-basket-info' + dopClassServices + (index == 0 ? ' bx-soa-item-tr-first' : '')},
					children: cols
				})
			);
			...
			if(withServices){	
				...
				basketItemsNode.appendChild(BX.create('TR', {props: {className: 'services-item-tr-padding'}, 
					children: [BX.create('TD', {props: {className: 'services-item-td-padding'},attrs:{'colspan': countCols}, text: ''})]}
				));
			}
			...
			if (hiddenColumns.length)
			{
				basketItemsNode.appendChild(
					BX.create('TR', {
						...
						children: [
							BX.create('TD', {
								props: {className: 'bx-soa-item-td'},attrs:{'colspan': countCols},
								children: [
									BX.create('A', {
		...
		createServicesBasketItem: function(item, countCols)
		{
			...
			var arrServicesDiv = [];
			var classImgPadding = this.options.showPreviewPicInBasket || this.options.showDetailPicInBasket ? ' need_img_padding ' : '';
			...
			for (var key in servicesInfo) {
				var infoHtml =  [
					...
					BX.create('SPAN', {props: {className: 'services_order_item_quantity'}, html: servicesInfo[key]['QUANTITY']+"&nbsp;"+servicesInfo[key]['MEASURE_TEXT']}),
				...
				if(servicesInfo[key]['NEED_SHOW_OLD_SUM'] === 'Y'){
					priceHtml.push(BX.create('SPAN', {props: {className: 'price_discount'}, html: servicesInfo[key]['SUM_BASE_FORMATED']}));
				}
				arrServicesDiv.push(BX.create('TR', {
					props: {className: 'services_order_item ' + classImgPadding},
					children: [
						BX.create('TD', {props: {className: 'services_order_item_info'},attrs:{'colspan': countCols-1}, 
							children: [BX.create('DIV', {props: {className: 'services_order_item_info_inner'}, children: infoHtml})]
						}),
		...
		editActiveRegionBlock: function(activeNodeMode)
		{
			var node = activeNodeMode ? this.regionBlockNode : this.regionHiddenBlockNode,
				...
				propsNodeCol, propsIterator, property, locationInput;
			...
			else
			{
				...
				regionContent.appendChild(propsNodeCol);
		...
		getDeliveryLocationInput: function(node)
		{
			var currentProperty, locationId, altId, location, k, altProperty,
				...
				labelTextHtml, label, input, altNode, locationInput;
			...
			if (location && location[0] && location[0].output)
			{
				...
				currentLocation.HTML = currentLocation.HTML.replace('class="dropdown-field"', 'class="dropdown-field" readonly');
				currentLocation.HTML = currentLocation.HTML.replace('<div class="bx-ui-sls-clear" title="Отменить выбор"></div>', '');
		...
		editActiveDeliveryBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				//$(deliveryContent).append('1111111111111111111');

				var btnNotMyDeliveryHtml = '<div class="btn btn-default btn-lg animate-load" data-event="jqm" data-param-form_id="NO_MY_CITY" data-name="no_my_city" style="display: none;">Нет моего города</div>';
				$(deliveryContent).append(btnNotMyDeliveryHtml);
		...
		getDeliveryExtraServices: function(delivery)
		{
			...
			for (i in delivery.EXTRA_SERVICES)
			{
				...
				if(currentService['PARAMS']['CUSTOM'] == 'Y'){
		...
		editTotalBlock: function()
		{
			...
			var total = this.result.TOTAL,
				...
				showOrderButton = this.params.SHOW_TOTAL_ORDER_BUTTON === 'Y', totalName;
			...
			if(this.result.SERVICES_IN_ORDER){
				totalName = BX.message('SOA_SUM_SUMMARY_WITH_SERVICES');
			} else {
				totalName = BX.message('SOA_SUM_SUMMARY');
			}

			this.totalInfoBlockNode.appendChild(this.createTotalUnit(totalName, priceHtml, params));
			...
			if (this.options.showTaxList)
			{
				for (i = 0; i < total.TAX_LIST.length; i++)
				{
					...
					var nameFromatted = '';
					if(total.TAX_LIST[i].CODE == 'VAT'){
						nameFromatted = 'Сумма НДС:';
					}
					else{
						nameFromatted = total.TAX_LIST[i].NAME + (!!total.TAX_LIST[i].VALUE_FORMATED ? ' ' + total.TAX_LIST[i].VALUE_FORMATED : '') + ':';
					}
