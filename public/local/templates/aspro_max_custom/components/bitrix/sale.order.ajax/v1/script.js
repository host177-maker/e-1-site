...
$( document ).ready(function() {
	...
	window.kladrInit = function($cityName = false){
		let  $city = $('input[data-id=city]'),
		$street = $('input[data-id=street]'),
		$building = $('input[data-id=building]');
		jQuery.fias.token='Azs7NksZfYr3Zf4tEfKbdRtF6Ed8Rsni';
		jQuery.fias.url='https://kladr-api.com/api.php';
		$.fias.setDefault({
		...
		if ($cityName) {
			//здесь делаем фокус на городе и улице, чтобы fias заработал
			if(jQuery('input[data-id="city"]').length > 0) {
				window.kladrStartForCity();
			}
		}
