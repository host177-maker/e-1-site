...
<?
if(!empty($arRegion['ID']) && $_POST['ajax'] != 'y'){
	$arItemFilter['PROPERTY_LINK_REGION'] = $arRegion['ID'];
	$GLOBALS[$arParams['FILTER_NAME']]['PROPERTY_LINK_REGION'] = $arRegion['ID'];
}
$arItemSelect = array('ID', 'NAME', 'IBLOCK_ID', 'DETAIL_PAGE_URL', 'IBLOCK_SECTION_ID', 'PROPERTY_MAP', 'PROPERTY_PHONE', 'PROPERTY_SCHEDULE', 'PROPERTY_METRO', 'PROPERTY_EMAIL', 'PROPERTY_ADDRESS');
...
<?
$iSelectedSectionId = $iSelectedSectionParentId = 0;
foreach($arItems as $arItem){
	$iSelectedSectionId = $arItem['IBLOCK_SECTION_ID'];
	$iSelectedSectionParentId = $arAllSections['CHILD_SECTIONS'][$iSelectedSectionId]['IBLOCK_SECTION_ID'];
	break;
}?>
		...
		<div class="contacts maxwidth-theme" itemscope itemtype="http://schema.org/Organization">
			<div class="row">
						...
						<div class="row">
							<div class="col-lg-6 col-md-4 col-sm-6">
								<select class="<?=($bHasChildSections ? 'region' : 'city');?>">
									...
									<option value="0" selected><?=Loc::getMessage('CHOISE_ITEM_NEW', array('#ITEM_NEW#' => ($bHasChildSections ? Loc::getMessage('REGION_NEW') : Loc::getMessage('CITY_NEW'))))?></option>
									<?foreach($arAllSections['ALL_SECTIONS'] as $arSection):?>
										<option<?if($arSection['SECTION']['ID'] == $iSelectedSectionParentId):?> selected<?endif;?> value="<?=$arSection['SECTION']['ID'];?>"><?=$arSection['SECTION']['NAME'];?></option>
									<?endforeach;?>
								</select>
							</div>
								...
								<div class="col-lg-6 col-md-4 col-sm-6">
									<select class="city">
										...
										<?foreach($arAllSections['CHILD_SECTIONS'] as $arSection):?>
											<option style="display:none;"<?if($arSection['ID'] == $iSelectedSectionId):?> selected<?endif;?> disabled="disabled" value="<?=$arSection['ID'];?>" data-parent_section="<?=$arSection['IBLOCK_SECTION_ID'];?>"><?=$arSection['NAME'];?></option>
										<?endforeach;?>
									</select>
								</div>
							<?endif;?>
				...
				<div class="col-lg-<?=($bHasSections ? 6 : 12);?> col-md-12 info-row">
					...
					<div class="hidden">
						<?if($arRegion):?>
							<?if(is_array($arRegion["PROPERTY_ADDRESS_VALUE"]) && $arRegion["PROPERTY_ADDRESS_VALUE"]["TEXT"]):?>
								<span itemprop="address"><?=$arRegion["PROPERTY_ADDRESS_VALUE"]["TEXT"];?></span>
							<?elseif(!empty($arRegion["PROPERTY_ADDRESS_VALUE"])):?>
                                <span itemprop="address"><?=$arRegion["PROPERTY_ADDRESS_VALUE"]["TEXT"];?></span>
                            <?endif;?>
						<?else:?>
...
<div class="ajax_items">
	...
	<?else:?>
		...
		<div class="maxwidth-theme">
			<div class="row">
				<div class="col-md-12">
					<p>В вашем городе нет салонов Е1, но Вы можете заказать шкаф на сайте, и мы с удовольствием доставим Вам его.</p>
				</div>
			</div>
		</div>
	<?endif;?>
