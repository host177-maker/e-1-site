...
//start e1 fix
COrwoFunctions::PrepareCatalogItemArray($arResult, 'detail', $arParams);
//end e1 fix
...
if(\BuyRestriction::isRestrictedGoods($arResult['ID'], $arResult['IBLOCK_SECTION_ID'], $arResult['IBLOCK_ID'])){
	$arResult['RESTRICTION'] = true;
}
/*stores product*/
...
if($arParams['USE_ADDITIONAL_GALLERY'] === 'Y'){
	...
	if($arResult['OFFERS'] && $arParams['ADDITIONAL_GALLERY_OFFERS_PROPERTY_CODE']){
		foreach($arResult['OFFERS'] as &$arOffer){
			...
			// Получаем из ТП только детальную фотку
	...
	// Убираем дупликаты фото из массива ТП
	foreach ($arOffersAdditionalGallery as $offerKey => $arOfferGallery) {
		foreach ($arResult['ADDITIONAL_GALLERY'] as $galleryKey => $arGallery) {
			if (
				$arGallery['DETAIL']['ID'] == $arOfferGallery[0]['DETAIL']['ID']
				|| $arGallery['DETAIL']['SRC'] == $arOfferGallery[0]['DETAIL']['SRC']
				|| $arGallery['DETAIL']['FILE_NAME'] == $arOfferGallery[0]['DETAIL']['FILE_NAME']
			) {
				unset($arOffersAdditionalGallery[$offerKey]);
				break;
			}
		}
	}
...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
	{
		...
		//start e1 fix
		if(isset($arResult['ADDITIONAL_GALLERY'][$arOffer['ID']])){
			$j = count($arOffer['MORE_PHOTO']);
			foreach($arResult['ADDITIONAL_GALLERY'][$arOffer['ID']] as $i => $arImage){
				$arImage['DETAIL']["BIG"]['src'] = CFile::GetPath($arImage['DETAIL']['ID']);
				$arImage['DETAIL']["SMALL"] = $arImage['PREVIEW'];
				$arImage['DETAIL']["THUMB"] = $arImage['THUMB'];
				$arOffer['MORE_PHOTO'][] = $arImage['DETAIL'];
			}
		}
	...
	//$arSortFields = array('SKU_SORT_BASIC_CONFIGURATION' => true);
	Collection::sortByColumn($arResult['OFFERS'], $arSortFields);
	//task_32348, add regions for corner module, update price
	global $arRegion;
	//start e1 fix
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
	{
		if(isset($arOffer['PROPERTIES']['PRICE_SBORKA_SHKAFA']) && empty($arOffer['PROPERTIES']['PRICE_SBORKA_SHKAFA']['VALUE'])){
			$arOffer['PROPERTIES']['PRICE_SBORKA_SHKAFA']['VALUE'] = 0;
			$arOffer['PROPERTIES']['PRICE_SBORKA_SHKAFA']['~VALUE'] = 0;
			$arOffer['PROPERTIES']['PRICE_SBORKA_SHKAFA']['DISPLAY_VALUE'] = 0;
			$arResult['OFFERS'][$keyOffer]['DISPLAY_PROPERTIES']['PRICE_SBORKA_SHKAFA'] = $arOffer['PROPERTIES']['PRICE_SBORKA_SHKAFA'];
		}
		if (!empty($arRegion["PROPERTY_REGION_GROUP_CORNER_MODULE_VALUE_XML_ID"])) {
			//we check whether the property exists for trade offers and if there is a value, then we update the price, which goes depending on the group of regions (siberia or moscow)
			if (isset($arOffer['PROPERTIES'][$arRegion["PROPERTY_REGION_GROUP_CORNER_MODULE_VALUE_XML_ID"]]["VALUE"]) && !empty($arOffer['PROPERTIES'][$arRegion["PROPERTY_REGION_GROUP_CORNER_MODULE_VALUE_XML_ID"]]["VALUE"])) {
				$arOffer['PROPERTIES']['PRICE_UM']['VALUE'] = $arOffer['PROPERTIES'][$arRegion["PROPERTY_REGION_GROUP_CORNER_MODULE_VALUE_XML_ID"]]["VALUE"];
				$arOffer['PROPERTIES']['PRICE_UM']['~VALUE'] = $arOffer['PROPERTIES'][$arRegion["PROPERTY_REGION_GROUP_CORNER_MODULE_VALUE_XML_ID"]]["VALUE"];
				$arOffer['PROPERTIES']['PRICE_UM']['DISPLAY_VALUE'] = $arOffer['PROPERTIES'][$arRegion["PROPERTY_REGION_GROUP_CORNER_MODULE_VALUE_XML_ID"]]["VALUE"];
				$arResult['OFFERS'][$keyOffer]['DISPLAY_PROPERTIES']['PRICE_UM'] = $arOffer['PROPERTIES']['PRICE_UM'];
			}
		}
	}
	//end e1 fix
			...
			if (!empty($arOffer['DISPLAY_PROPERTIES']))
			{
				...
				foreach ($arOffer['DISPLAY_PROPERTIES'] as &$arOneProp)
				{
					...
					$arSKUProps[] = array(
						'NAME' => $arOneProp['NAME'],
						'VALUE' => (
							is_array($arOneProp['DISPLAY_VALUE']) && count($arOneProp['DISPLAY_VALUE'] ?: []) > 1
...
if(empty($arResult["SERVICES"])){
	COrwoFunctions::getServicesItems($arResult, $arParams);
}
...
if(!empty($arResult['ID'])){
	$dbElement = CIBlockElement::GetList(Array("SORT"=>"ASC"), Array("ID" => $arResult['ID']), false, false, array('SHOW_COUNTER'));
	if ($arElement = $dbElement->GetNext()){
		if(!empty($arElement['SHOW_COUNTER'])){
...
//формирование массива с данными для екомерса
$arResult['ECOM_DATA'] = [];
$arEcomData = [
    ...
    'index' => 1,
    'quantity' => $arResult['CATALOG_MEASURE_RATIO'],
];
if (!empty($arEcomData)) {
    $arResult['ECOM_DATA'] = $arEcomData;
}

//создаем тп c нужным ключом
...
if($arResult["OFFERS"] && $arResult['OFFERS_CURRENT']){
	$arOffer  = $arResult['OFFERS_CURRENT'];
	$arOfferEcomData['price'] = $arOffer['MIN_PRICE']['DISCOUNT_VALUE'] ? $arOffer['MIN_PRICE']['DISCOUNT_VALUE'] : $arOffer['MIN_PRICE']['VALUE'];
	$arOfferEcomData['item'] = [
	...
	$arResult['ECOM_DATA'] = $arOfferEcomData;
}

if ($arResult['PROPERTIES']['RATING']['VALUE']) {
	$intRationValue = (int)$arResult['PROPERTIES']['RATING']['VALUE'];
	// Ensure value is between 0 and 5
	$arResult['RATING_VALUE'] = max(0, min(5, $intRationValue));
	unset($intRationValue);
}
...
if (is_object($cp))
{
	...
	$cp->arResult['OFFERS_CURRENT'] = (!empty($arResult['OFFERS_CURRENT'])) ? $arResult['OFFERS_CURRENT'] : $endOffer;
	$cp->arResult['SERIYA_SHKAFA'] = $arResult['PROPERTIES']['SERIYA_SHKAFA']['VALUE'];
	$cp->arResult['TIP_SHKAFA'] = $arResult['PROPERTIES']['TIP_SHKAFA']['VALUE'];
    if(!empty($arResult['ECOM_DATA'])){
        $cp->arResult['ECOM_DATA'] = $arResult['ECOM_DATA'];
    }
    $cp->SetResultCacheKeys(array('OFFERS_CURRENT', 'OFFERS', 'OFFERS_SELECTED', 'SERIYA_SHKAFA', 'TIP_SHKAFA', 'ECOM_DATA'));
}
