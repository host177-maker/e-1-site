...
if (isset($arResult['ECOM_DATA']) && !empty($arResult['ECOM_DATA'])) {
    ...
    <script>
        ...
        dataLayer.push({
            ...
            ecommerce: {
                items: <?=json_encode($arResult['ECOM_DATA'], JSON_UNESCAPED_UNICODE);?>
            },
        });
    </script>
    <?
...
if(!empty($GLOBALS[$arParams['FILTER_NAME']]['OFFERS']) && !empty($arResult['ITEMS_IDS'])){
	// $arItems = CCatalogSKU::getOffersList(
	// 	$arResult['ITEMS_IDS'],
	// 	$arParams['IBLOCK_ID'],
	...
	// 	foreach ($arItem as $sOfferKey => $arOffer) {
	// 		$arItems[$key]['OFFERS'][$arOffer['ID']] = $arOffer;
	// 		unset($arItems[$key][$sOfferKey]);
	// 	}
	// }

	// $arCurrentOffers = array();
	// foreach ($arItems as $key => $arItem) {
	// 	//if(count($arItem['OFFERS']) == 1){
	// 		$arOffer = current($arItem['OFFERS']);
	// 		$arOffer['DETAIL_PAGE_URL'] = str_replace(array('#PRODUCT_URL#', '#ID#'), array($arResult['ITEMS_URLS'][$arOffer['PARENT_ID']], $arOffer['ID']), $arOffer['DETAIL_PAGE_URL']);
	// 		$arCurrentOffers[$arOffer['PARENT_ID']] = $arOffer;
	// 	//}
	// }
}
?>
...
<script>
	$(document).ready(function (){
		...
		<?if(!empty($arCurrentOffers)):?>
			// var currentOffers = <?=CUtil::PhpToJSObject($arCurrentOffers);?>;
			// for (var i in currentOffers) {
			// 	var offerUrl = currentOffers[i]['DETAIL_PAGE_URL'];
