...
foreach ($arResult['ITEMS'] as $key => $arItem) {
    if ($arItem['PROPERTIES']['GLAVNAYA_STRANITSA']['VALUE_XML_ID'] == 'true') {
        $strValIdProducts[] = $arItem["ID"];
    }
}

$arInfoIblock = CCatalogSKU::GetInfoByProductIBlock($arResult['IBLOCK_ID']);
if (is_array($arInfoIblock)) {
    $arSelect = array('NAME', 'ID', 'XML_ID', 'PREVIEW_PICTURE', 'DETAIL_PICTURE', 'DETAIL_PAGE_URL', 'PROPERTY_*', 'CATALOG_GROUP_RETAIL');
    $rsOffers = CIBlockElement::GetList(array(), array('IBLOCK_ID' => $arInfoIblock['IBLOCK_ID'], 'PROPERTY_' . $arInfoIblock['SKU_PROPERTY_ID'] => $strValIdProducts, 'PROPERTY_IS_HOME_OFFERS_VALUE' => 'Да'), false, false, $arSelect);
    while ($obElement = $rsOffers->GetNextElement()) {
        $elOffer = $obElement->GetFields();
        $elOffer['PROPERTIES'] = $obElement->GetProperties();
        $arSkuProductLink = CCatalogSku::GetProductInfo($elOffer['ID']);
        $arResultPrices = CIBlockPriceTools::GetCatalogPrices($elOffer['IBLOCK_ID'], $arParams['PRICE_CODE'][0]);
        $arPriceProduct = CCatalogProduct::GetOptimalPrice($elOffer['ID']);

        foreach ($arResultPrices as &$value) {
            $arPriceTypeID[] = $value['ID'];
        }

        $elOffer['PRICE_MATRIX'] = CatalogGetPriceTableEx($elOffer['ID'], 0, $arPriceTypeID, 'Y');
        $elOffer['ITEM_PRICE_SELECTED'] = 0;
        $elOffer['ITEM_PRICES'][] = array(
            'UNROUND_BASE_PRICE' => $arPriceProduct['RESULT_PRICE']['UNROUND_BASE_PRICE'],
            'UNROUND_PRICE' => $arPriceProduct['DISCOUNT']['PRICE'],
            'BASE_PRICE' => $arPriceProduct['RESULT_PRICE']['BASE_PRICE'],
            'PRICE' => $arPriceProduct['DISCOUNT']['PRICE'],
            'ID' => $elOffer['ID'],
            'PRICE_TYPE_ID' => $arPriceProduct['RESULT_PRICE']['PRICE_TYPE_ID'],
            'CURRENCY' => $arPriceProduct['RESULT_PRICE']['CURRENCY'],
            'DISCOUNT' => ($arPriceProduct['RESULT_PRICE']['BASE_PRICE'] - $arPriceProduct['DISCOUNT']['PRICE']),
            'PERCENT' => (($arPriceProduct['RESULT_PRICE']['BASE_PRICE'] - $arPriceProduct['DISCOUNT']['PRICE']) * 100) / $arPriceProduct['RESULT_PRICE']['BASE_PRICE'],
            'QUANTITY_FROM' => "",
            'QUANTITY_TO' => "",
            'QUANTITY_HASH' => "",
            'MEASURE_RATIO_ID' => '',
            'PRINT_BASE_PRICE' => FormatCurrency($arPriceProduct['RESULT_PRICE']['BASE_PRICE'], $arPriceProduct['RESULT_PRICE']['CURRENCY']),
            'RATIO_BASE_PRICE' => $arPriceProduct['RESULT_PRICE']['BASE_PRICE'],
            'PRINT_RATIO_BASE_PRICE' => FormatCurrency($arPriceProduct['RESULT_PRICE']['BASE_PRICE'], $arPriceProduct['RESULT_PRICE']['CURRENCY']),
            'PRINT_PRICE' => FormatCurrency($arPriceProduct['DISCOUNT']['PRICE'], $arPriceProduct['RESULT_PRICE']['CURRENCY']),
            'RATIO_PRICE' => $arPriceProduct['DISCOUNT']['PRICE'],
            'PRINT_RATIO_PRICE' => FormatCurrency($arPriceProduct['DISCOUNT']['PRICE'], $arPriceProduct['RESULT_PRICE']['CURRENCY']),
            'PRINT_DISCOUNT' => FormatCurrency(($arPriceProduct['RESULT_PRICE']['BASE_PRICE'] - $arPriceProduct['DISCOUNT']['PRICE']), $arPriceProduct['RESULT_PRICE']['CURRENCY']),
            'RATIO_DISCOUNT' => ($arPriceProduct['RESULT_PRICE']['BASE_PRICE'] - $arPriceProduct['DISCOUNT']['PRICE']),
            'PRINT_RATIO_DISCOUNT' => FormatCurrency(($arPriceProduct['RESULT_PRICE']['BASE_PRICE'] - $arPriceProduct['DISCOUNT']['PRICE']), $arPriceProduct['RESULT_PRICE']['CURRENCY']),
            'MIN_QUANTITY' => 1,
        );
        $arResult['OFFERS_HOME'][$arSkuProductLink['ID']][$elOffer['ID']] = $elOffer;
    }

}


foreach ($arResult["ITEMS"] as $k => $arValProducts) {
    if (isset($arResult["OFFERS_HOME"][$arValProducts["ID"]]) && !empty($arResult["OFFERS_HOME"][$arValProducts["ID"]])) {
        unset($arResult["ITEMS"][$k]);
        foreach ($arResult["OFFERS_HOME"][$arValProducts["ID"]] as $key => $value) {
            $arResult["ITEMS"][] = $value;
        }

    }
}
//end home page , offers 
//start e1 fix
...
if (!empty($arResult['ITEMS'])) {
    ...
    foreach ($arResult['ITEMS'] as $key => $arItem) {
        ...
        if ($arItem['CATALOG'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS'])) {
            ...
            foreach ($arItem['OFFERS'] as $keyOffer => $arOffer) {
                ...
                //start e1 fix
                // if(!empty($arOffer['DISPLAY_PROPERTIES']['COLOR_REF']['DISPLAY_VALUE'])){
                // 	$arOffer['PROPERTIES']['COLOR_REF']['DISPLAY_VALUE'] = $arOffer['DISPLAY_PROPERTIES']['COLOR_REF']['DISPLAY_VALUE'];
                // }

                // if( $bNeedFindPicture || $bChangeName){
                // 	if($bNeedFindPicture){
                ...
                // 	}
                // 	if($bChangeName){
                // 		$sSeriyaShkafa = $sTipShkafa = $sColorRef = '';
                // 		if(!empty($arItem['PROPERTIES']['SERIYA_SHKAFA']['VALUE'])){
                // 			$sSeriyaShkafa = $arItem['PROPERTIES']['SERIYA_SHKAFA']['VALUE'];
                // 		}
                // 		if(!empty($arItem['PROPERTIES']['TIP_SHKAFA']['VALUE'])){
                // 			$sTipShkafa = str_replace('дверные', 'дверный', $arItem['PROPERTIES']['TIP_SHKAFA']['VALUE']);
                // 		}
                // 		if(!empty($arOffer['PROPERTIES']['COLOR_REF']['DISPLAY_VALUE'])){
                // 			$sColorRef = $arOffer['PROPERTIES']['COLOR_REF']['DISPLAY_VALUE'];
                // 		}
                // 		if(!empty($sSeriyaShkafa) && !empty($sTipShkafa) && !empty($sColorRef)){
                // 			$sOffersName = 'Шкаф-купе ' . $sSeriyaShkafa . ' ' . $sTipShkafa . ' ' . $sColorRef;
                // 			$arOffer['NAME'] = $sOffersName;
                // 			$arOffer['~NAME'] = $sOffersName;

                // 			$arResult['ITEMS'][$key]['OFFERS'][$keyOffer]['NAME'] = $sOffersName;
                // 			$arResult['ITEMS'][$key]['OFFERS'][$keyOffer]['~NAME'] = $sOffersName;
                // 			$arItem['OFFERS'][$keyOffer]['NAME'] = $sOffersName;
                // 			$arItem['OFFERS'][$keyOffer]['~NAME'] = $sOffersName;
                // 		}
                // 	}
                // }
    ...
    //формирование массива с данными по таварам для екомерса
    $arResult['ECOM_DATA'] = [];
    if (isset($arResult['ITEMS']) && !empty($arResult['ITEMS'])) {
        $arEcomData = [];
        foreach ($arResult['ITEMS'] as $index => &$arItem) {
            if (!empty($arItem["OFFERS"])) {
                if ($arItem["OFFERS"][$arItem["OFFERS_SELECTED"]]) {
                    $ecomItem = [
                        'item_name' => $arItem["OFFERS"][$arItem["OFFERS_SELECTED"]]['NAME'],
                        'item_id' => $arItem["OFFERS"][$arItem["OFFERS_SELECTED"]]['ID'],
                        'price' => $arItem["OFFERS"][$arItem["OFFERS_SELECTED"]]['MIN_PRICE']['DISCOUNT_VALUE'] ? $arItem["OFFERS"][$arItem["OFFERS_SELECTED"]]['MIN_PRICE']['DISCOUNT_VALUE'] : $arItem["OFFERS"][$arItem["OFFERS_SELECTED"]]['MIN_PRICE']['VALUE'],
                        'item_list_name' => $arParams['TITLE_BLOCK'],
                        'index' => $index + 1,
                        'quantity' => $arItem["OFFERS"][$arItem["OFFERS_SELECTED"]]['CATALOG_MEASURE_RATIO'],
                    ];
                    $arItem["OFFERS"][$arItem["OFFERS_SELECTED"]]['ECOM_ITEM_DATA'] = $ecomItem;
                    $arItem['ECOM_ITEM_DATA'] = $ecomItem;
                    $arEcomData[] = $ecomItem;
                }elseif($arItem["OFFER_ID_SELECTED"]){
                    foreach ($arItem["OFFERS"] as &$arOffer){
                        if($arOffer['ID'] == $arItem["OFFER_ID_SELECTED"]){
                            $ecomItem = [
                                'item_name' => $arOffer['NAME'],
                                'item_id' => $arOffer['ID'],
                                'price' => $arOffer['MIN_PRICE']['DISCOUNT_VALUE'] ? $arOffer['MIN_PRICE']['DISCOUNT_VALUE'] : $arOffer['MIN_PRICE']['VALUE'],
                                'item_list_name' => $arParams['TITLE_BLOCK'],
                                'index' => $index + 1,
                                'quantity' => $arOffer['CATALOG_MEASURE_RATIO'],
                            ];
                            $arOffer['ECOM_ITEM_DATA'] = $ecomItem;
                            $arItem['ECOM_ITEM_DATA'] = $ecomItem;
                            $arEcomData[] = $ecomItem;
                        }
                    }
                ...
                $ecomItem = [
                    ...
                    'item_id' => $arItem['ID'],
                    'price' => $arItem['MIN_PRICE']['DISCOUNT_VALUE'] ? $arItem['MIN_PRICE']['DISCOUNT_VALUE'] : $arItem['MIN_PRICE']['VALUE'],
                    'item_list_name' => $arParams['TITLE_BLOCK'],
                    'index' => $index + 1,
    ...
    $this->__component->SetResultCacheKeys(array("ITEMS_IDS", "ITEMS_URLS", "SELECTED_OFFERS", 'ECOM_DATA'));
}
?>