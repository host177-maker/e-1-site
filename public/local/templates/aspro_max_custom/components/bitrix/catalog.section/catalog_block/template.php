	...
	<? if ($arParams["AJAX_REQUEST"] != "Y") : ?>
		...
		<div class="top_wrapper items_wrapper <?= $templateName; ?>_template <?= $arParams['IS_COMPACT_SLIDER'] ? 'compact-catalog-slider' : '' ?>">
			...
			<? foreach ($arResult["ITEMS"] as $arItem) { ?>
				...
				<? ob_start(); ?>
				<div class="sa_block" data-id-t="<?= $arItem['OFFERS'][0]['ID'] ?>" data-fields='<?= Json::encode($arParams["FIELDS"]) ?>' data-stores='<?= Json::encode($arParams["STORES"]) ?>' data-user-fields='<?= Json::encode($arParams["USER_FIELDS"]) ?>'>
					<?= $arQuantityData["HTML"]; ?>
				...
				<div class="item-title">
					<a href="<?= $arItem["DETAIL_PAGE_URL"] ?>" onclick="onProductClick(<?= \CUtil::PHPToJSObject($arItem['ECOM_ITEM_DATA']) ?>)" class="<?= $bBigBlock && $bFonImg ? '' : 'dark_link' ?> option-font-bold font_sm"><span><?= $elementName; ?></span></a>
				</div>
				...
				<div class="cost prices clearfix">
						...
						<? if ($arCurrentSKU) : ?>
							...
							<div class="js_price_wrapper">
								...
								if(empty($arItem["PRICES"])){
									$widgetPrice = $arItem['MIN_PRICE']['DISCOUNT_VALUE'];
								} else {
									$widgetPrice = (array_values($arItem["PRICES"]))[0]["DISCOUNT_VALUE"];
								}
								?>
							</div>
							<? if ($arCurrentSKU) : ?>
					...
					<? else : ?>
						...
						<? endif; ?>
					<? endif; ?>
					<yandex-pay-badge merchant-id="5ef67561-d34e-45e2-bb25-8320a4653548" type="cashback" amount="<?= $widgetPrice ?>" size="m" theme="light" align="left" color="primary" />
									...
									<div class="offer_buy_block">
										...
										<div class="counter_wrapp ce_cmp_visible">
											<div id="<?= $arItemIDs["ALL_ITEM_IDS"]['BASKET_ACTIONS']; ?>" class="button_block wide">
												<a class="btn btn-default basket read_more" rel="nofollow" data-item="<?= $arItem['ID'] ?>" href="<?= $arItem['DETAIL_PAGE_URL'] ?>" onclick="onProductClick(<?= \CUtil::PHPToJSObject($arItem['ECOM_ITEM_DATA']) ?>)"><?= Loc::getMessage('CATALOG_READ_MORE') ?></a>
											</div>
										</div>
									</div>
									<? if (isset($arCurrentSKU['PRICE_MATRIX']) && $arCurrentSKU['PRICE_MATRIX']) // USE_PRICE_COUNT
				...
				<div class="col-lg-<?= $col; ?><? if ($bBigBlock) : ?> col-md-8 col-sm-12 col-xs-12<? else : ?> col-md-4 col-sm-6 col-xs-6<? endif; ?> col-xxs-12 item item-parent item_block<?= ($bBigBlock ? ' big' : ''); ?> <?= ($arParams['SET_LINE_ELEMENT_COUNT'] ? 'custom-line' : ''); ?> <?= (!empty($arItem["PROPERTIES"]["SHOW_ICON_SPECIAL"]["VALUE"]) ? ' special ' : ''); ?>" data-id="<?= $arItem["ID"]; ?>">
					...
					<div class="catalog_item_wrapp catalog_item item_wrap main_item_wrapper<?= ($bBigBlock ? ' big' : ''); ?> <?= ($bFonImg ? '' : ' product_image') ?> <?= ($arItem["OFFERS"] ? 'has-sku' : '') ?>" id="<?= $arItem["strMainID"] ?>">
						<div class="inner_wrap <?= $arParams["TYPE_VIEW_BASKET_BTN"] ?>">
							...
							<? if ($bBigBlock) : ?>
								<? if ($bFonImg) : ?>
									<a class="absolute-full-block lazy absolute-full-block_bg_center darken-bg-animate" href="<?= $arItem['DETAIL_PAGE_URL']; ?>" onclick="onProductClick(<?= \CUtil::PHPToJSObject($arItem['ECOM_ITEM_DATA']) ?>)" data-src="<?= $imageSrc ?>" data-bg="<?= $imageSrc ?>" style="background-image:url(<?= \Aspro\Functions\CAsproMax::showBlankImg($imageSrc); ?>)"><span></span></a>
								<? endif; ?>
							<? endif; ?>
