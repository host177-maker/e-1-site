    ...
    <div class="content_wrapper_block <?=$templateName;?> services_in_product <?=($bCompact ? 'services_compact' : '')?>">		
        <div class="services-items">
            ...
            <?foreach($arResult["ITEMS"] as $key => $arItem){?>
                ...
                ?>
                <div class="services-item bordered rounded3 <?=($services_in_basket ? 'services_on' : '')?> <?=( !$services_in_basket && $showAllInSlide && $counter > $count_to_display ? 'hide_service' : 'show_service' )?> <?=($services_in_basket && $showAllInSlide ? 'order_top_service' : '')?>" data-item_id ="<?=$arItem["ID"]?>" data-item-code="<?=$arItem['PROPERTIES']['SERVICE_PRICE']['VALUE'];?>" data-item="<?=$arItem["ID"]?>" data-iblockid="<?=$arItem["IBLOCK_ID"]?>" data-quantity="1">
                                ...
                                <div class="services-item__cost cost prices clearfix">
                                    ...
                                    <?if(!$services_in_basket):?>                                   
                                        <?if($arItem["ITEM_PRICES"]){?>
                                            <?$min_price_id=0;?>
                                            <?\Aspro\Functions\CAsproMaxItem::showItemPrices($arParams, $arItem["ITEM_PRICES"], $strMeasure, $min_price_id, 'Y', false, false, true);?>
                                        <?}?>
                                    <?else:?>
