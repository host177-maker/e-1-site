...
<?if($arResult["ITEMS"]):?>
	...
	//$sSliderResponsiveParams = '{"0":{"items": 2},"600":{"items": 2},"768":{"items": 3},"1200":{"items": 3}}';
	$sSliderParams = '{"nav": true, "autoplay" : false,  "autoplayTimeout" : "3000", "smartSpeed":1000, ' . (count($arResult["ITEMS"]) > 4 ? "\"loop\": true," : "") . ' "responsiveClass": true, "responsive":{"0":{"items": 2},"600":{"items": 2},"768":{"items": 3},"1200":{"items": 3}}}';
	if(!empty($arParams['SLIDER_PARAMS_STRING'])){
		$sSliderParams = $arParams['SLIDER_PARAMS_STRING'];
	}
	?>
	...
	<div class="top_wrapper items_wrapper <?=$templateName;?>_template <?=$arParams['IS_COMPACT_SLIDER'] ? 'compact-catalog-slider' : ''?>">
		...
		<div class="catalog_block items row <?=$arParams['IS_COMPACT_SLIDER'] ? 'swipeignore mobile-overflow' : ''?> margin0 <?=$bHasBottomPager ? 'has-bottom-nav' : ''?> js_append ajax_load block flexbox<?=($bSlide ? ' owl-carousel owl-theme owl-bg-nav visible-nav short-nav hidden-dots swipeignore ' : '');?>"<?if($bSlide):?>data-plugin-options='<?=$sSliderParams;?>'<?endif;?>>
	<?endif;?>
		<?
		...
		<?foreach($arResult["ITEMS"] as $arItem){?>
			...
			if ($arItem['OFFERS']) {
				$arQuantityData = Quantities::GetProductQuantity($arItem['OFFERS']);
			} else {
