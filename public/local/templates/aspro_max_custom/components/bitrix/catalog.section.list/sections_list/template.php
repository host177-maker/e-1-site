							...
							<td class="section_info toggle">
								<ul>
									<li class="name<?=($bBigBlock ? ' text-center' : '');?>">
                                        ...
                                        } else if ($arItems['UF_IS_LINK'] && $arItems['UF_LINK']) {
                                        ...
                                        <a href="<?=$href?>"
                                           class="dark_link"><span class="<?=($bSlideBlock ? 'font_mlg' : 'font_md');?>"><?=$arItems["NAME"]?></span></a>
									...
									<?if($arItems["SECTIONS"]):?>
										...
										<?foreach($arItems["SECTIONS"] as $key => $arItem):?>
                                            <li class="sect<?=(++$i > $iVisibleItemsMenu ? ' collapsed' : '');?><?=($bSlideBlock ? ' col-sm-6 font_sm' : ' font_xs');?>">
                                                <? if($arItem['UF_IS_FILE'] && $arItem['UF_FILE']): ?>
                                                    <a href="<?=CFile::GetPath($arItem["UF_FILE"])?>" class="<?=($bSmallBlock ? 'muted777' : '')?>"><?=$arItem["NAME"]?></a>
                                                <? elseif ($arItem['UF_IS_LINK'] && $arItem['UF_LINK']): ?>
                                                    <a href="<?=$arItem['UF_LINK']?>" class="<?=($bSmallBlock ? 'muted777' : '')?>"><?=$arItem["NAME"]?></a>
                                                <? else: ?>
                                                    <a href="<?=$arItem["SECTION_PAGE_URL"]?>" class="<?=($bSmallBlock ? 'muted777' : '')?>">
                                                        <?=$arItem["NAME"]?><?=($arItem["ELEMENT_CNT"]?'&nbsp;<span class="'.($bSmallBlock ? 'dark-color' : '').'">'.$arItem["ELEMENT_CNT"].'</span>':'');?>
                                                    </a>
                                                <?endif;?>
                                            </li>
										<?endforeach;?>
