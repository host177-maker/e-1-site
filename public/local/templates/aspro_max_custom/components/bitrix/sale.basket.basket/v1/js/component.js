; (function () {
	...
	BX.Sale.BasketComponent = {
		...
		resizeServicesBasketTimeout: function ($time = 10) {
		...
		fillTotalBlocks: function () {
			...
			if (totalNodes && totalNodes.length) {
				...
				if (totalTemplate) {
					...
					if (!activeSession) {
						console.log('allSum', this.result.allSum);
						...
						const paymentData = {
							...
							totalAmount: this.result.allSum,
						...
						const currentPrice = this.result.allSum;
		...
		sendRequest: function (action, data) {
			...
			var basketItemId = $(clickTarget).closest('.basket-items-list-item-container').attr('data-id');
			var services = $('.basket-services-list-item-container[data-id="' + basketItemId + '"]').find('.services-item');
			var quantity = $(clickTarget).closest('.basket-items-list-item-container').find('.basket-item-amount-filed').attr('data-value')
			...
			BX.ajax({
				...
				onsuccess: BX.delegate(function (result) {
					...
					if (addService) {
						this.addServices(basketItemId, services, quantity, result.BASKET_DATA.BASKET_ITEM_RENDER_DATA);
					}
					addService = false;
		...
		quantityPlus: function (target) {
			...
			if (itemData) {
				...
				//custom
				if (itemData.ECOM_ITEM_JSON) {
					let dataObject = JSON.parse(itemData.ECOM_ITEM_JSON);
					if (dataObject) {
						ecommerceOnBasketAdd(dataObject);
					}
				}

				var basketItemId = $(target).closest('.basket-items-list-item-container').attr('data-id');
				var parentId = $('.basket-services-list-item-container[data-id="' + basketItemId + '"]').find('.services_in_basket_page').attr('data-parent_product');

				//update services
				this.quantitySend(quantity, parentId);
			}
		},

		quantitySend: function (quantity, parentId) {
			timer = setTimeout(() => {
				this.updateServices(quantity, parentId);
			}, 1500)
		},

		skuChangeSend: function (itemId, props, oldProps, itemData) {
			timer2 = setTimeout(() => {
				this.actionPool.changeSku(itemId, props, oldProps);
		...
		quantityMinus: function (target) {
			...
			if (itemData) {
				...
				if (itemData.ECOM_ITEM_JSON) {
					let dataObject = JSON.parse(itemData.ECOM_ITEM_JSON);
					if (dataObject) {
						ecommerceOnBasketDelete(dataObject);
					}
				}
		...
		quantityChange: function () {
			...
			if (itemData) {
				...
				var basketItemId = $(BX.proxy_context).closest('.basket-items-list-item-container').attr('data-id');
				var parentId = $('.basket-services-list-item-container[data-id="' + basketItemId + '"]').find('.services_in_basket_page').attr('data-parent_product');
		...
		bindSkuEvents: function (node, data) {
			...
			//добавление в корзину сборки
			//ставим событие клика, при клике получаем данные и отправляем
			var serviceClickAdd = this.getEntities(node, 'add_services_custom');
			var blockEntitiesService, ij, lj, ijj, lj;
			for (ij = 0, lj = serviceClickAdd.length; ij < lj; ij++) {
				blockEntitiesService = this.getEntities(serviceClickAdd[ij], 'add_services');

				for (ijj = 0, ll = blockEntitiesService.length; ijj < ll; ijj++) {
					BX.bind(blockEntitiesService[ijj], 'click', BX.proxy(this.addServiceCustom, this));
				}
			}
		},
		addServiceCustom: function (blockEntitiesService) {
			var itemData = this.getItemDataByTarget(BX.proxy_context);
			if (itemData) {
				var quantityField, quantity;
				...
				quantityField = BX(this.ids.quantity + itemData.ID);
				quantity = this.getCorrectQuantity(itemData, quantityField.value);
				var services = $('.basket-services-list-item-container[data-id="' + itemData.ID + '"]').find('.services-item-sborka');
				//отключаем клик на чекбокс, чтоб запрос не задвоился при многократном нажатии
				$('.basket-item-property-custom-checkbox[data-id="' + itemData.ADD_SERVICE_HTML + '"]').css('pointer-events', 'none');
				
				console.log('addServiceCustom:', itemData);
				
				if(services.length) {
					
					$.ajax({
                        url: arAsproOptions['SITE_DIR'] + 'ajax/item.php?delete_assembly=' + itemData.ADD_SERVICE_HTML,
                        type: 'post',
                        success: function (html) {
                            updateBasketInfoServices(true);
                            setTimeout(() => {
                                getActualBasketInAssembly(
                                    null,
                                    null,
                                    null,
                                    true,
                                    itemData.ADD_SERVICE_HTML,
                                    itemData.PRODUCT_ID
                                );
                            }, 2500);
                        },
					...
					itemData.SERVICES_CUSTOM_CHECKED = true;

					$.ajax({
						url: arAsproOptions['SITE_DIR'] + 'ajax/item.php?add_service=' + itemData.ADD_SERVICE_HTML,
						type: 'post',
						data: {
							parentId    : itemData.PRODUCT_ID,
							parentPrice : itemData.FULL_PRICE,
							iblockid    : itemData.IBLOCK_SERVICE_ID,
							quantity    : quantity ? quantity : itemData.QUANTITY,
						},
						success: function (html) {
							updateBasketInfoServices(true);
							setTimeout(() => {
								getActualBasketInAssembly(null, null, null, false, itemData.ADD_SERVICE_HTML, itemData.PRODUCT_ID);
							}, 2500)
						}
					});
				}
		...
		updateServices: function (quantity, parentId) {
			$.ajax({
				url: arAsproOptions['SITE_DIR'] + 'ajax/item.php?update_service=' + parentId,
				type: 'post',
				data: {
					quantity: quantity,
				},
		...
		addServices: function (basketItemId, services, quantity, result) {
			let parentId;

			if (result != false) {
				result.forEach((element) => {
					if (element.ID == basketItemId) {
						parentId = element.PRODUCT_ID;
					}
				});
			} else {
				parentId = $('.basket-services-list-item-container[data-id="' + basketItemId + '"]').find('.services_in_basket_page').attr('data-parent_product');
			}
			...
			if (services)
			{
				services.each(function(index) {
					let service = $(this);
					$.ajax({
						url: arAsproOptions['SITE_DIR'] + 'ajax/item.php?add_service=' + service.data("item"),
						type: 'post',
						data: {
							parentId: parentId,
							iblockid: service.data("iblockid"),
							quantity: quantity ? quantity : service.data("quantity"),
						},
						success: function (html) {
