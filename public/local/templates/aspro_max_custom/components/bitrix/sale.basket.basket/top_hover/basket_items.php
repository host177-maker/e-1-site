...
$bHasServices = false;
$arResult = \E_1\Prices::resultPricesModify($arResult);
...
<?endif;?>
	<div class="basket_wrap">
		<div class="items_wrap srollbar-custom">
										...
										<div class="prices <?=( $bTypeColumn ? 'notes' : '' );?>">
											...
											<div class="cost prices">
												<?if( doubleval($arItem["DISCOUNT_PRICE_PERCENT"]) > 0 && $bDiscountColumn){?>
													<?//здесь малая корзина находится, пометил на всякий случай?>
            ...
            <?if($arResult['RESTRICTED']):?>
                <div class="item-restriction _detail _basket">Покупка возможна только при условии приобретения шкафа.</div>
            <?endif;?>
		</div>
		...
		<div class="foot flexbox flexbox--row">
			<div class="pull-left">
				<span class="wrap_remove_button basket_action">
					<?if($normalCount){?>
						<span class="colored_theme_hover_text remove_all_basket AnDelCanBuy cur" data-type="basket" onclick="ecommerceDeleteAllBasket(<?= \CUtil::PHPToJSObject($arResult['ECOM_DATA'])?>)">
