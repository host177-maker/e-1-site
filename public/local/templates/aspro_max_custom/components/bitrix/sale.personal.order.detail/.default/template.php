													...
													<div class="col-md-12 col-md-12 col-sm-12 sale-order-detail-payment-options-shipment">
														<div>
															...
															<div class="col-md-7 col-sm-7 sale-order-detail-payment-options-methods-shipment-list">
																...
																<?if (isset($arResult["ORDER_PROPS"])):?>
																	<?foreach ($arResult["ORDER_PROPS"] as $property):?>
																		<?if ($property['IS_ADDRESS'] == 'Y'):?>
																			<?if(!empty($arResult['PERSON_TYPE_ID']) && !empty($arResult['DELIVERY_ID'])){
																				$order = \Bitrix\Sale\Order::load($arResult['ID']);
																				$dbOrderProp = \Bitrix\Sale\Property::getList([
																				    'select' => ['*'], 
																				    'filter' => [
																				        'REL_DLV.ENTITY_ID' => $arResult['DELIVERY_ID'],
																				...
																				if ($arOrderProp = $dbOrderProp->fetch())
																				{
																					if(!empty($arOrderProp['ID'])){
																						...
																						if (!is_null($propertyValue)) {
																							$sStoreAddress = $propertyValue->getField('VALUE');
																							$property["VALUE"] = $sStoreAddress;
																						}
																					}
																				}
																			}?>
