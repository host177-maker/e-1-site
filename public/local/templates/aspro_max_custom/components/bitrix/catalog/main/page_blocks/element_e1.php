...
<? $APPLICATION->SetPageProperty("offVisitGetBlue", true); ?>
...
<div class="product-view <?=($arParams["SHOW_UNABLE_SKU_PROPS"] != "N" ? "show_un_props" : "unshow_un_props");?> main_item_wrapper product-view--type2">
	...
	<?
	$bTemplateParamAjaxJs = true;
	$bTemplateParamClearOffersData = true;
	$bAjaxGetJs = false;
	$sCurrentBasePage = '';
	if($_POST["get_template_js"] && $_POST["get_template_js"] == "y"){
		$bAjaxGetJs = true;
	}
	if($bAjaxGetJs){
		$bTemplateParamAjaxJs = false;
	...
	<?
		// $arParams["CACHE_TYPE"] = 'N';
		// if($APPLICATION->GetCurPage() == '/catalog/shkafy_kupe/3_kh_dvernye/82962/' && !$bAjaxGetJs){
		// 	$arParams["CACHE_TYPE"] = 'A';
		// 	$arParams["CACHE_TIME"] = 180;
		// }
		// Убираем каноникал для подключенных страниц
		...
		if (!empty($aSitemapEnable['ENABLE_DIR'])) {
			foreach ($aSitemapEnable['ENABLE_DIR'] as $strDir) {
				if ((stripos($APPLICATION->GetCurPage(false), $strDir) !== false) && (strpos($APPLICATION->GetCurPage(false), '/catalog/') !== false)) {
					$arParams["DETAIL_SET_CANONICAL_URL"] = "N";
				}
			}
		}
	?>
...
$arElementParams = Array(
	...
	"CLEAR_TEMPLATE_DATA_OFFERS" => $bTemplateParamClearOffersData,
