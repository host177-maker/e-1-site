...
<?if($itemsCnt):?>
			...
			<?}?>
				...
				global $arrFilterMeta;
  				$arrFilterMeta   =   array("ACTIVE" => "Y", "PROPERTY_URL" => $sUrlPage);
				//добавляем мета теги
				$context = \Bitrix\Main\Application::getInstance()->getContext();
				$request = $context->getRequest();
				//проверка на ajax запрос, чтобы при нажатии показать еще не было тегов, fix
				if (!$request->isAjaxRequest()) {
					...
					$APPLICATION->IncludeComponent("bitrix:news.list","meta_tags",Array(
						...
						"AJAX_OPTION_HISTORY" => "N",
						"AJAX_OPTION_ADDITIONAL" => "",
						"CURRENT_SECTION" => $arSectionFilter
						)
					);
				}
				?>
...
<?if(isset($isAjaxFilter) && $isAjaxFilter):?>
	<?global $APPLICATION;?>
	<?
	$arAdditionalData['TITLE'] = htmlspecialcharsback(\CMaxEvents::getSeoTitle());
	if($arSeoItem)
	{
		$postfix = '';
	}
	$arAdditionalData['WINDOW_TITLE'] = htmlspecialcharsback(\COrwoSubdomain::makeCurrentCityText($APPLICATION->GetTitle('title').$postfix));
    $arAdditionalData['meta'] = \CMaxEvents::setMeta(true);
    ?>
	<script type="text/javascript">
		...
		BX.addCustomEvent("onAjaxSuccessFilter", function tt(e){
            ...
            if (arAjaxPageData.meta.title && arAjaxPageData.meta.description){
                    BX.ajax.UpdateWindowTitle(arAjaxPageData.meta.title);
                    document.querySelector('meta[name="description"]').setAttribute("content", arAjaxPageData.meta.description);
            }
			<?endif;?>
