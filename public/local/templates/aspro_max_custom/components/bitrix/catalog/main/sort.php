...
<div class="filter-panel sort_header view_<?=$display?> <?=($bShowCompactHideLeft && $bHideLeftBlock ? 'show-compact' : '' );?> <?=(!$bShowSortInFilter ? 'show-normal-sort' : '' );?>">
	<?if('Y' === $arParams['USE_FILTER']):?>
		...
		<div class="filter-panel__filter pull-left filter-<?=strtolower($arTheme['FILTER_VIEW']['VALUE']);?>  <?=($bHideLeftBlock && !$bShowCompactHideLeft ? 'filter-panel__filter--visible' : '');?>">
			...
			<button class="bx_filter_search_reset btn-link-text font_sxs colored_theme_text_with_hover<?=($bActiveFilter && $bActiveFilter[1] != 'clear' ? '' : ' hidden');?>" type="reset" id="del_filter" name="del_filter" data-href="<?echo str_replace('/filter/clear/apply/','/',$arResult["SEF_DEL_FILTER_URL"]);?>">
				...
				if($arSortPrices == "MINIMUM_PRICE" || $arSortPrices == "MAXIMUM_PRICE"){
					...
					if($arSortPrices == "REGION_PRICE")
					{
						...
						if($arRegion)
						{
							...
							{
								//ставим сортировку по свойству, если в конфиге стоит true
								$aFilterMinimumPriceEnable = \Cosmos\Config::getInstance()->getParam("PROP_MIN_PRICE");
								if ($aFilterMinimumPriceEnable['ENABLE']) {
									$arAvailableSort["PRICE"] =  array((($arRegion["PROPERTY_FILTER_SIB_PRICE_VALUE"] === 'Y' || $arRegion['CODE'] === 'novosibirsk') ? 'PROPERTY_MINIMUM_PRICE_SIB' : 'PROPERTY_MINIMUM_PRICE'), 'desc');
								} else {
					...
					<?foreach($arAvailableSort as $key => $arVals):?>
						<?foreach($arOrder as $value):?>
							<?$newSort = $sort_order == 'desc' ? 'asc' : 'desc';
								...
								if($_REQUEST['sort'] == $key && $value == $_REQUEST['order'])
								?>
