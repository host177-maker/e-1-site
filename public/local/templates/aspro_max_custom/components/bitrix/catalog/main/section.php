...
foreach ($sections as $sectionCode) {
    $res = CIBlockSection::GetList(array(), array('CODE' => $sectionCode));
    $section = $res->Fetch();
    $catalogSections[] = $section['NAME'];
}
if (!empty($catalogSections)){
    $APPLICATION->SetPageProperty('CATALOG_SECTION_NAME',$catalogSections);
}
...
if ($section) {
	...
	$arSubSectionFilter = array(
		...
		"ACTIVE" => "Y",
		"GLOBAL_ACTIVE" => "Y",
		//убираем разделы, которые выводятся как тег
		'UF_IS_TAG' => 0,
	);
	...
	//end e1 fix
    $itemsCnt = CMaxCache::CIBlockElement_GetList(
            array(
                    "CACHE" => array(
	...
	if (!$typeTmpSKU || !$viewTmpFilter || !$arSection["DISPLAY"] || !$bSetElementsLineRow 
		...
		|| !$linkedBanners	|| !$linkedBannersPos || $linkedBannersRows || $linkedBannersRowsMobile
		) {
