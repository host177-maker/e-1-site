...
if ($arParams['NOT_SLIDER'] == 'Y' && $arResult['ITEMS'] && $arParams['STAFF_IBLOCK_ID']) {
    ...
    foreach ($arResult['ITEMS'] as $key => $arItem) {
        ...
        if ($arStaffsID) {
            $arResult['STAFF'] = CMaxCache::CIblockElement_GetList(['CACHE' => ['TAG' => CMaxCache::GetIBlockCacheTag($arParams['STAFF_IBLOCK_ID']), 'MULTI' => 'N', 'GROUP' => 'ID']], ['IBLOCK_ID' => $arParams['STAFF_IBLOCK_ID'], 'ID' => $arStaffsID], false, false, ['ID', 'NAME', 'PROPERTY_POST', 'PREVIEW_PICTURE']);
        }
    }
}
