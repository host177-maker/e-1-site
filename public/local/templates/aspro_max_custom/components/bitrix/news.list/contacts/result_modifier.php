...
if($arResult['ITEMS'])
{
	...
	foreach($arResult['SECTIONS'] as $si => $arSection) {
		foreach ($arSection['CHILD_IDS'] as $ci => $arChildSection) {
			if(!empty($arAllSections['CHILD_SECTIONS'][$ci]['NAME']))
				$arResult['SECTIONS'][$si]['SECTION']['NAME'] .= ', ' . $arAllSections['CHILD_SECTIONS'][$ci]['NAME'];
		}
	}
}
?>