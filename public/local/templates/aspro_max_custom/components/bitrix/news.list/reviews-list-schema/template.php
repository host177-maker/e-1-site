			...
			<?endif;?>
				<?foreach($arResult['ITEMS'] as $i => $arItem):?>
					...
					<div class="item-wrapper col-xs-12<?=(!$bOneItem ?  ' bg-fill-white bordered1 box-shadow1' : '');?>">
						<div class="item clearfix <?=($bLogo ? ' wlogo' : '')?> <?=(!$bImage ? 'no_img' : '')?> <?=(!$bOneItem ? ' bordered box-shadow' : '');?> <?=($notSlider ? ' rounded2 bordered' : '');?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>" itemscope itemtype="https://schema.org/Review">
								...
								<div class="top-info">
									<div class="wrap muted">
										...
										<?if(isset($arItem['DISPLAY_ACTIVE_FROM']) && $arItem['DISPLAY_ACTIVE_FROM']):?>
											<span class="date font_upper" itemprop="datePublished"><?=$arItem['DISPLAY_ACTIVE_FROM']?></span>
										<?endif;?>
									</div>
									<div class="title <?=($bCompact ? 'font_md' : 'font_lg');?> pull-left" itemprop="author">
										<?if($bDetailLink):?><a href="<?=$arItem['DETAIL_PAGE_URL']?>"><?endif;?>
										<?=$arItem['NAME'];?>
										<?if($bDetailLink):?></a><?endif;?>
									</div>
							...
							<?if(isset($arItem['FIELDS']['PREVIEW_TEXT']) && $arItem['FIELDS']['PREVIEW_TEXT']):?>
								<div class="body-info">
									...
									<div class="preview-text" itemprop="reviewBody"><?=$arItem['FIELDS']['PREVIEW_TEXT'];?></div>
