...
<?if($arResult['SECTIONS']):?>
	...
	<?if(!empty($arParams['MAN_BLOCK_TITLE'])):?>
		<div class="top_block">
			<h3><?=$arParams['MAN_BLOCK_TITLE'];?></h3>
			<?if(!empty($arParams['MAN_BLOCK_TITLE_ALL']) && !empty($arParams['MAN_BLOCK_TITLE_ALL_URL'])):?>
				<a href="<?=$arParams['MAN_BLOCK_TITLE_ALL_URL'];?>" class="pull-right font_upper muted"><?=$arParams['MAN_BLOCK_TITLE_ALL'];?></a>
			<?endif;?>
		</div>
	<?endif;?>

<div class="item-views items-list1 <?=($onlyImgPart ? 'only-img' : '')?> <?=($documentsMode ? 'documents-mode' : '');?> <?=($licensesMode ? 'licenses-mode' : '');?> <?=$arParams['VIEW_TYPE']?> <?=$arParams['VIEW_TYPE']?>-type-block <?=($arParams['SHOW_TABS'] == 'Y' ? 'with_tabs' : '')?> <?=($arParams['IMAGE_POSITION'] ? 'image_'.$arParams['IMAGE_POSITION'] : '')?> <?=($templateName = $component->{'__parent'}->{'__template'}->{'__name'})?>" itemscope itemtype="https://schema.org/FAQPage">
	<?// top pagination?>
		...
		<?endif;?>
				...
				<div class="<?=($arParams['SHOW_TABS'] == 'Y' ? 'tab-content' : 'group-content')?>">
					...
					<?foreach($arResult['SECTIONS'] as $SID => $arSection):?>
						...
						?>
						<div id="<?=$this->GetEditAreaId($arSection['ID'])?>" class="<?if($arParams['SHOW_TABS'] == 'Y'):?>tab-pane<?endif;?> <?=(!$si++ || !$arSection['ID'] ? 'active' : '')?>">
								...
								<?foreach($arSection['ITEMS'] as $i => $arItem):?>
									...
									<?elseif($arParams['VIEW_TYPE'] == 'accordion'):?>
										<div class="accordion-type-1">
											<div class="item item-accordion-wrapper <?=($bImage ? '' : ' wti')?>  bordered box-shadow" id="<?=$this->GetEditAreaId($arItem['ID'])?>" itemscope itemprop="mainEntity" itemtype="https://schema.org/Question">
												...
												<div id="accordion<?=$arItem['ID']?>_<?=$arSection['ID']?>" class="panel-collapse collapse" itemscope itemprop="acceptedAnswer" itemtype="https://schema.org/Answer">
													<div class="accordion-body">
														<div class="row">
															<?if(!$bImage):?>
																<div class="col-md-12"><div class="text" itemprop="text"><?=$textPart?></div></div>
															<?elseif($arParams["IMAGE_POSITION"] == "right"):?>
																<div class="col-md-9"><div class="text" itemprop="text"><?=$textPart?></div></div>
