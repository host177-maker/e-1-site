...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_REQUEST["Reindex"] == "Y") {
    ...
    if (Option::get('abr.search', 'USE_ANMARTO') === 'Y') {
        $res = CAnmartoSearch::ReIndexAll($bFull, COption::GetOptionInt("search", "max_execution_time"), $NS, $_REQUEST["clear_suggest"] === "Y");
    } else {
    ...
    if (is_array($res)):
        ...
        CAdminMessage::ShowMessage(array(
            ...
            "DETAILS" => GetMessage("SEARCH_REINDEX_TOTAL") . " <b>" . $res["CNT"] . "</b><br>
				" . $path . "
				<a id=\"continue_href\" onclick=\"savedNS=" . $jsNS . "; ContinueReindex(); return false;\" href=\"" . htmlspecialcharsbx("abr.search_search_reindex.php?Continue=Y&lang=" . urlencode(LANGUAGE_ID) . $urlNS) . "\">" . GetMessage("SEARCH_REINDEX_NEXT_STEP") . "</a>",
            "HTML" => true,
            "TYPE" => "PROGRESS",
        ));
        ?>
    ...
    <script language="JavaScript">
        ...
        function DoNext(NS) {
            ...
            if (!stop) {
                ShowWaitWindow();
                BX.ajax.post(
                    'abr.search_search_reindex.php?' + queryString,
                    NS,
                    function (result) {
