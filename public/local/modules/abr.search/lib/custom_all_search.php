...
class CAnmartoAllSearch extends CDBResult
{
        ...
        if (is_array($result)) {
            ...
            if (is_array($aParamsEx) && !empty($aParamsEx)) {
                foreach ($aParamsEx as $aParamEx) {
                    $strSqlWhere = CAnmartoSearch::__PrepareFilter($aParamEx, $bIncSites);
                    if ($strSqlWhere != "")
            ...
            $strSqlWhere = CAnmartoSearch::__PrepareFilter($arParams, $bIncSites);
            if ($strSqlWhere != "")
    ...
    {
        $perm = CAnmartoSearch::CheckPermissions("sc.ID");
    ...
    function Fetch()
    {
        ...
        if ($r) {
            ...
            if (!isset($arSite[$site_id])) {
                $by = '';
                $order = '';
                $filter = array("ID" => $site_id);
                $rsSite = CSite::GetList($by, $order, $filter);
    ...
    public static function GetGroupCached()
    {
        ...
        if (!is_array($SEARCH_CACHED_GROUPS)) {
            ...
            $by = 'id';
            $order = 'asc';
            $db_groups = CGroup::GetList($by, $order);
    ...
    public static function ReIndexAll($bFull = false, $max_execution_time = 0, $NS = array(), $clear_suggest = false)
    {
        ...
        if (
            ...
            $by = '';
            $order = '';
            $r = CSite::GetList($by, $order, $arFilter);
            while ($arR = $r->Fetch()) {
                $path = rtrim($arR["DIR"], "/");
            ...
            foreach ($arLangDirs as $arR) {
                ...
                //for every folder
                CAnmartoSearch::RecurseIndex(array($site, $path), $max_execution_time, $NS, $arData);
                if (
                    $max_execution_time > 0
                    && $NS["MODULE"] <> ''
                ) {
        ...
        foreach (GetModuleEvents("search", "OnReindex", true) as $arEvent) {
            if ($arEvent['TO_MODULE_ID'] == 'iblock') {
                ...
                $r = &$oCallBack;
                $arResult = CAllIBlockAnmarto::OnSearchReindex($NS, $r, "Index");
                if (is_array($arResult)) //old way
                {
                    foreach ($arResult as $arFields) {
                        ...
                        if ($ID <> '') {
                            unset($arFields["ID"]);
                            $NS["CNT"]++;
                            $NS["ID"] = $arFields['PARAM2'] . '.' . $ID;
                            $idx = CAnmartoSearch::Index($arEvent["TO_MODULE_ID"], $ID, $arFields, false, $NS["SESS_ID"]);
                            if (is_array($arFields)) {
                                $arFields['ITEM_ID'] = $ID;
                                $arFields['MODULE_ID'] = $arEvent["TO_MODULE_ID"];
                                if(is_array($idx)) {
                                    $arFields['ID'] = $idx['ID'];
                                }else{
                                    $arFields['ID'] = $idx;
                    ...
                    if ($max_execution_time > 0 && !empty($arResult)) {
                        if (!empty($arData)) {
                            $arValues = [];
                            ...
                            $chunks = array_chunk($arValues, 100);
                            foreach ($chunks as $chunk) {
                                CSearchFullText::getInstance()->replaceAll($chunk);
                            }
                        }
                        $DB->Commit();
                        return array(
                ...
                if (is_array($arResult)) //old way
                {
                    foreach ($arResult as $arFields) {
                        ...
                        if ($ID <> '') {
                            unset($arFields["ID"]);
                            $NS["CNT"]++;
                            CAnmartoSearch::Index($arEvent["TO_MODULE_ID"], $ID, $arFields, false, $NS["SESS_ID"]);
                            $arData[] = $arFields;
                        }
                    }
                } else  //new method
        ...
        if (!$bFull) {
            CAnmartoSearch::DeleteOld($NS["SESS_ID"], $NS["MODULE_ID"], $NS["SITE_ID"]);
        }

        if (!empty($arData)) {
            CSearchFullText::getInstance()->replaceAll($arData);
        }
        $DB->Commit();
    ...
    {
        if ($bFull) {
            CAnmartoSearch::DeleteForReindex($MODULE_ID);
        }
            ...
            if (is_array($arResult)) //old way
            {
                foreach ($arResult as $arFields) {
                    ...
                    if ($ID <> '') {
                        unset($arFields["ID"]);
                        $NS["CNT"]++;
                        $arFields[] = CAnmartoSearch::Index($arEvent["TO_MODULE_ID"], $ID, $arFields, false, $NS["SESS_ID"]);
                    }
                }
                CSearchFullText::getInstance()->replaceAll($arFields);
            } else  //new way
            {
        ...
        if (!$bFull) {
            CAnmartoSearch::DeleteOld($NS["SESS_ID"], $MODULE_ID, $NS["SITE_ID"]);
        }
    }
    ...
    public static function Index($MODULE_ID, $ITEM_ID, $arFields, $bOverWrite = false, $SEARCH_SESS_ID = "")
    {
        ...
        if (array_key_exists("SITE_ID", $arFields)) {
			...
			";
            $r = $DB->Query($strSql, false, "File: " . __FILE__ . "<br>Line: " . __LINE__);
            $arFields["Anmarto_RANK_SQL"] = $strSql;
            if ($arResult = $r->Fetch())
                $arFields["Anmarto_RANK"] = $arResult["RANK"];
        }
        ...
        if ($arResult = $r->Fetch()) {
            ...
            if ($bTitle && $bBody && $arFields["BODY"] == '' && $arFields["TITLE"] == '') {
                ...
                CAnmartoSearch::CleanFreqCache($ID);
            ...
            if (is_set($arFields, "PARAMS"))
                CAnmartoAllSearch::SetContentItemParams($ID, $arFields["PARAMS"]);

            if (count($arGroups) > 0)
                CAnmartoAllSearch::SetContentItemGroups($ID, $arGroups);

            if (is_set($arFields, "SITE_ID")) {
                CAnmartoSearch::UpdateSite($ID, $arFields["SITE_ID"]);
            }
            ...
            if ($bBody || $bTitle || $bTags) {
                ...
                $arFields["SEARCHABLE_CONTENT"] = CAnmartoSearch::KillEntities(ToUpper($content));
            }
            ...
            if (array_key_exists("TITLE", $arFields)) {
                if (
                    ...
                    CAnmartoSearch::IndexTitle($arFields["SITE_ID"], $ID, $arFields["TITLE"]);
            }

            if ($bTags && ($arResult["TAGS"] != $arFields["TAGS"])) {
                CSearchTags::CleanCache("", $ID);
                $DB->Query("DELETE FROM b_search_tags WHERE SEARCH_CONTENT_ID = " . $ID, false, "File: " . __FILE__ . "<br>Line: " . __LINE__);
                CAnmartoSearch::TagsIndex($arFields["SITE_ID"], $ID, $arFields["TAGS"]);
            }

            foreach (GetModuleEvents("search", "OnBeforeIndexUpdate", true) as $arEvent)
                ExecuteModuleEventEx($arEvent, array($ID, $arFields));

            CAnmartoSearch::Update($ID, $arFields);
...
//            CSearchFullText::getInstance()->replace($ID, $arFields);
            ...
            $ID = CAnmartoSearch::Add($arFields);
...
//            CSearchFullText::getInstance()->replace($ID, $arFields);
    ...
    public static function ReindexFile($path, $SEARCH_SESS_ID = "")
    {
        ...
        if (!CAnmartoSearch::CheckPath($file_rel_path))
            return 0;
        ...
        $title = CAnmartoSearch::KillTags(trim($arrFile["TITLE"]));

        if ($title == '')
            return 0;

        //strip out all the tags
        $filesrc = CAnmartoSearch::KillTags($arrFile["CONTENT"]);

        $arGroups = CAnmartoSearch::GetGroupCached();
        $arGPerm = array();
        ...
        //save to database
        $arFields = CAnmartoSearch::Index("main", $item_id,
            array(
                "SITE_ID" => $file_site,
    ...
    public static function RecurseIndex($path = array(), $max_execution_time = 0, &$NS, &$arData = [])
    {
        if (!is_array($path))
            return 0;
        ...
        foreach ($d->GetChildren() as $dir_entry) {
            ...
            if ($dir_entry->IsDirectory()) {
                ...
                if (
                    ...
                    if (CAnmartoSearch::CheckPath($path_file . "/") !== false) {
                        if (CAnmartoSearch::RecurseIndex(array($site, $path_file), $max_execution_time, $NS, $arData) === false)
                            return false;
                    }
                } else //all done
                {
                ...
                if (
                    ...
                    $aData = CAnmartoSearch::ReindexFile(array($site, $path_file), $NS["SESS_ID"]);
                    if (!empty($aData['ID'])) {
    ...
    {
        $str = CAnmartoSearch::RemovePHP($str);
    ...
    {
        $arData = CAnmartoSearch::ReindexFile(array($site, $path));
        CSearchFullText::getInstance()->replaceAll([$arData]);
    }
        ...
        if (file_exists($DOC_ROOT . $path)) {
            ...
            if (is_dir($DOC_ROOT . $path)) {
                ...
                while (false !== ($file = @readdir($handle))) {
                    ...
                    if (is_dir($DOC_ROOT . $full_file) || CAnmartoSearch::CheckPath($full_file))
                        CAnmartoSearch::OnChangeFilePermissions(array($site, $full_file), array(), array(), $arGroups);
                }
            } else//if(is_dir($DOC_ROOT.$path))
            {
                ...
                if ($ar = $rs->Fetch()) {
                    ...
                    CAnmartoAllSearch::SetContentItemGroups($ar["ID"], $arNewGroups);
                }
            } //if(is_dir($DOC_ROOT.$path))
        }//if(file_exists($DOC_ROOT.$path))
    }
    ...
    public static function DeleteIndex($MODULE_ID, $ITEM_ID = false, $PARAM1 = false, $PARAM2 = false, $SITE_ID = false)
    {
        ...
        if ($PARAM1 !== false && $PARAM2 !== false) {
            $strSqlWhere = CAnmartoSearch::__PrepareFilter(array(
                "MODULE_ID" => $MODULE_ID,
                $op . "ITEM_ID" => $ITEM_ID,
                array(
                    "=PARAM1" => $PARAM1,
    ...
    public static function Update($ID, $arFields)
    {
        ...
        if (array_key_exists("PERMISSIONS", $arFields)) {
            ...
            CAnmartoSearch::SetContentItemGroups($ID, $arNewGroups);
            $bUpdate = true;
        }

        if (array_key_exists("PARAMS", $arFields)) {
            CAnmartoSearch::SetContentItemParams($ID, $arFields["PARAMS"]);
            $bUpdate = true;
        }
    ...
    public static function ChangeIndex($MODULE_ID, $arFields, $ITEM_ID = false, $PARAM1 = false, $PARAM2 = false, $SITE_ID = false)
    {
        ...
        $rs = $DB->Query($strSql, false, "File: " . __FILE__ . "<br>Line: " . __LINE__);
        while ($ar = $rs->Fetch()) {
            CAnmartoSearch::Update($ar["ID"], $arFields);
        }
    }
    ...
    public static function ChangeSite($MODULE_ID, $arSite, $ITEM_ID = false, $PARAM1 = false, $PARAM2 = false, $SITE_ID = false)
    {
        ...
        $r = $DB->Query($strSql, false, "File: " . __FILE__ . "<br>Line: " . __LINE__);
        while ($arR = $r->Fetch()) {
            CAnmartoSearch::Update($arR["ID"], array("SITE_ID" => $arSite));
        }
    }
    ...
    public static function ChangePermission($MODULE_ID, $arGroups, $ITEM_ID = false, $PARAM1 = false, $PARAM2 = false, $SITE_ID = false, $PARAMS = false)
    {
		...
		", false, "File: " . __FILE__ . "<br>Line: " . __LINE__);
        while ($arR = $rs->fetch()) {
            CAnmartoSearch::Update($arR["ID"], array("PERMISSIONS" => $arGroups));
        }
    }
}

class CSearchAnmartoCallback
{
    var $MODULE = "";
