...
class CAnmartoSearch extends CAnmartoAllSearch
{
    ...
    function MakeSQL($query, $strSqlWhere, $strSort, $bIncSites, $bStem)
    {
        ...
        if ($bIncSites && $bStem)
        {
            ...
            if (!preg_match("/(sc|sct)./", $query))
            {
				...
				WHERE
				".CAnmartoSearch::CheckPermissions("sc.ID")."
				".(count($this->Query->m_stemmed_words) > 1? "": "
        ...
        foreach ($arLang as $lang => $value)
        {
            ...
            if (BX_SEARCH_VERSION > 1)
            {
                $arPos = stemming($sContent, $lang, /*$bIgnoreStopWords*/
                    ...
                    true);
                CAnmartoSearch::RegisterStem($arDoc);
            }
            ...
            if ($docLength > 0)
            {
                ...
                if (BX_SEARCH_VERSION > 1)
                {
                    ...
                    {
                        $stem_id = CAnmartoSearch::RegisterStem($word);
                        //This is almost impossible, but happens
                        if ($stem_id > 0)
                            $strSqlValues .= ",\n("
                                .$ID
                                .", '".$sql_lang."'"
                                .", ".CAnmartoSearch::RegisterStem($word)
