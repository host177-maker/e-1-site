    ...
    public static function OnSearchReindex($NS=Array(), $oCallback=NULL, $callback_method="")
    {
        ...
        while($arIBlock = $dbrIBlock->Fetch())
        {
            ...
            if($arIBlock["INDEX_ELEMENT"]=='Y' && ($strNSFilter2 !== false))
            {
                ...
                while($arIBlockElement = $dbrIBlockElement->GetNext())
                {
                    $DETAIL_URL =
                    ...
                    $elem = CIBlockElement::GetList([], ['=ID' => $arIBlockElement['ID']], false, false, ['ID', 'DETAIL_PAGE_URL'])->GetNext();
                    if(!empty($elem)){
                        $DETAIL_URL = $elem['DETAIL_PAGE_URL'] ?? '';
                    }
