...
class ConditionType{
	static function OnIBlockPropertyBuildList(){
		return array(
			'PROPERTY_TYPE' => 'L',
			'USER_TYPE' => 'SAsproModalConditionsTypeMax',
			'DESCRIPTION' => Loc::getMessage('MAX_MODAL_CONDITIONS_TYPES_PROP_TITLE'),
	...
	private static function addCss($arProperty){
		$GLOBALS['APPLICATION']->SetAdditionalCss('/bitrix/css/aspro.max/ConditionType.css');
	}
	...
	private static function addJs($arProperty, $dependProps){
		...
		$GLOBALS['APPLICATION']->AddHeadScript('/bitrix/js/aspro.max/ConditionType.js');
	}
}
