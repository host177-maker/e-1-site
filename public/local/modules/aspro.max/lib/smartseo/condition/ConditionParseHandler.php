            ...
            if (!empty($name)) {
                ...
                if ($name === 'SECTION_ID') {
                    if ($operator === '!') {
                        ...
                        $result['INCLUDE_SUBSECTIONS'] = isset($params['INCLUDE_SUBSECTIONS']) && $params['INCLUDE_SUBSECTIONS'] === 'N'
                              ? 'N' : 'Y';
        ...
        if (!empty($result)) {
                ...
                if (!empty($result[$name]) && is_array($result[$name])) {
                    ...
                    if (($ind = strpos($name, 'CondIBProp')) !== false) {
                        ...
                        if (!empty($catalogInfo)) {
                            if (
                              $catalogInfo['CATALOG_TYPE'] != \CCatalogSku::TYPE_CATALOG && $catalogInfo['IBLOCK_ID'] == $iblock
                            ) {
            ...
            if (!empty($subFilter) && !empty($catalogInfo)) {
                ...
                $result['=ID'] = \CIBlockElement::SubQuery(
                    'PROPERTY_' . $catalogInfo['SKU_PROPERTY_ID'], $offerPropFilter + $subFilter
                );
            }
        }
    }
