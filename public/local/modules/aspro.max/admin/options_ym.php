...
if($RIGHT >= "R"){
	...
	<?=CAdminMessage::ShowMessage(GetMessage("MAX_MODULE_CURL_INFO"));?>
	<?if(!count($arTabs)):?>
									...
									<?if(isset($arOption['DEPENDENT_PARAMS']) && $arOption['DEPENDENT_PARAMS']): //dependent params?>
										<?foreach($arOption['DEPENDENT_PARAMS'] as $key => $arValue):?>
											<?if(!isset($arValue['CONDITIONAL_VALUE']) || ($arValue['CONDITIONAL_VALUE'] && $arTab["OPTIONS"][$optionCode] == $arValue['CONDITIONAL_VALUE']))
												...
												$style = "style='display:none'";
											?>
											<tr data-optioncode="<?=$key;?>" class="depend-block <?=$key?>" <?=((isset($arValue['CONDITIONAL_VALUE']) && $arValue['CONDITIONAL_VALUE']) ? "data-show='".$arValue['CONDITIONAL_VALUE']."'" : "");?> data-parent='<?=$optionCode."_".$arTab["SITE_ID"]?>' <?=$style;?>><?=$moduleClass::ShowAdminRow($key, $arValue, $arTab, $arControllerOption, true);?></tr>
										<?endforeach;?>
									<?endif;?>
									<?
								}
			...
			<script type="text/javascript">
				$(document).ready(function() {
					$('form.max_options').submit(function(e) {
						...
						jsAjaxUtil.ShowLocalWaitWindow('id', 'max_options', true);
