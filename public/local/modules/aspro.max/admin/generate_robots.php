	...
	foreach($arSites as $key => $arSite){
		if($arTabsForView) {
			if(in_array($arSite['ID'], $arTabsForView)) {
				...
				$arItems = array();
				$rsItems = CIBlockElement::GetList(array("NAME" => "ASC"), array("ACTIVE" => "Y", "LID" => $optionsSiteID, "IBLOCK_CODE" => "aspro_max_regions"), false, false, array("ID", "NAME", "IBLOCK_ID", "PROPERTY_MAIN_DOMAIN"));
				while($arItem = $rsItems->Fetch())
				{
					$arItems[] = $arItem;
				}
				$bGenerate = ((Option::get("aspro.max", "REGIONALITY_TYPE", "ONE_DOMAIN", $optionsSiteID) == "SUBDOMAIN") && Option::get("aspro.max", "USE_REGIONALITY", "N", $optionsSiteID) == "Y");
				if($bGenerate)
					$bShowGenerate = true;
...
\tRewriteRule ^(.*)$ /robots.php [L]", $file);
	...
	<?else:?>
		<?if($bShowGenerate):?>
			<div class="adm-info-message"><?=GetMessage("MAX_MODULE_ROBOTS_INFO");?></div>
			<br>
			<br>
		<?endif;?>
		<?$tabControl->Begin();?>
		...
		foreach($arTabs as $key => $arTab)
		{
			...
			if($arTab["SITE_ID"])
			{
				...
				<tr>
					<td>
						<?=GetMessage("MAX_MODULE_ROBOTS_MAIN");?>							
					</td>
					<td style="width:50%;">
						...
						?><a class="adm-btn" href="<?=$href?>" name="edit" title="<?=GetMessage('MAX_MODULE_EDIT_ROBOTS')?>"><?=GetMessage('MAX_MODULE_EDIT_ROBOTS')?></a>
						
					</td>
				</tr>
				<?if($arTab["HAS_REGIONS"]):?>
					<tr class="heading"><td colspan="2"><?=GetMessage("MAX_MODULE_DOMAINS")?></td></tr>
						...
						foreach($arTab["ITEMS"] as $arItem):?>
							<tr>
								...
								<td style="width:50%;">
									...
									?><a class="adm-btn" href="<?=$href?>" name="edit" title="<?=GetMessage('MAX_MODULE_EDIT_ROBOTS')?>"><?=GetMessage('MAX_MODULE_EDIT_ROBOTS')?></a>
									<input type="button" name="generate" data-element_id="<?=$arItem["ID"];?>" class="submit-btn adm-btn-save" value="<?=GetMessage("MAX_MODULE_GENERATE_ROBOTS_SHORT")?>" title="<?=GetMessage("MAX_MODULE_GENERATE_ROBOTS_SHORT")?>">
				...
				<?else:?>
					<tr>
						<td style="width:100%;text-align:center;" colspan="2">
							<div class="adm-info-message"><?=GetMessage("MAX_MODULE_ROBOTS_ERROR");?></div>
						</td>
					</tr>
				<?endif;?>	
			<?}
		...
		if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults) && check_bitrix_sessid())
		{
			...
			<?$tabControl->Buttons();?>
			<?if($bShowGenerate):?>
				<input <?if($RIGHT < "W") echo "disabled"?> type="submit" name="Apply" class="submit-btn adm-btn-save" value="<?=GetMessage("MAX_MODULE_GENERATE_ROBOTS")?>" title="<?=GetMessage("MAX_MODULE_GENERATE_ROBOTS")?>">
			<?endif;?>
