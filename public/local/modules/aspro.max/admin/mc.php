...
if($_SERVER['REQUEST_METHOD'] === 'GET'){
	$GLOBALS['APPLICATION']->SetTitle(GetMessage('ASPRO_MAX_CONTROL_CENTER_TITLE'));
}
...
if($RIGHT >= 'R'){
	if(CModule::IncludeModule($moduleID)){
		if($_SERVER['REQUEST_METHOD'] === 'GET'){
			...
			$.ajax({
				...
				error: function(){
					...
					$('#aspro_admin_area').append('<div class="adm-info-message-wrap adm-info-message-red"><div class="adm-info-message"><div class="adm-info-message-title"><?=GetMessage('ASPRO_MAX_MODULE_CONTROL_CENTER_ERROR')?></div><div class="adm-info-message-icon"></div></div></div>');
				}
			});
			</script>
			<?
		...
		CAdminMessage::ShowMessage(GetMessage('ASPRO_MAX_MODULE_NOT_INCLUDED'));
	}
}
else{
