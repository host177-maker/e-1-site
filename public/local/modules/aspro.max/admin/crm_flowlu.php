...
if($RIGHT >= "R")
{
		...
		if($arTabsForView) {
			if(in_array($arSite['ID'], $arTabsForView)) {
				...
				$arTabs[] = array(
					...
					"ITEMS" => array(
						"CONFIG" => array(
							"TITLE" => Loc::getMessage("ASPRO_MAX_MODULE_CONFIG_FLOWLU"),
							"ITEMS" => array(
								"DOMAIN_FLOWLU" => array(
									"TYPE" => "text",
								...
								"TOKEN_FLOWLU" => array(
									...
									"HINT" => Loc::getMessage("ASPRO_MAX_MODULE_TOKEN_HINT")
								),
							)
						),
						"LINK" => array(
							"TITLE" => Loc::getMessage("ASPRO_MAX_MODULE_LINK_FLOWLU"),
							"ITEMS" => array(
								...
								"AUTOMATE_SEND_FLOWLU" => array(
									...
									"HINT" => Loc::getMessage("ASPRO_MAX_MODULE_AUTOMATE_SEND_FLOWLU_HINT")
								),
								"USE_LOG_FLOWLU" => array(
									"TYPE" => "checkbox",
									"VALUE" => Option::get($moduleID, "USE_LOG_FLOWLU", "N", $optionsSiteID),
									"HINT" => Loc::getMessage("ASPRO_MAX_MODULE_USE_LOG_FLOWLU_HINT")
								),
								"LEAD_NAME_FLOWLU_TITLE" => array(
									"TYPE" => "text",
									"VALUE" => Option::get($moduleID, "LEAD_NAME_FLOWLU_TITLE", Loc::getMessage("ASPRO_MAX_MODULE_LEAD_NAME_FLOWLU"), $optionsSiteID),
								),
	...
	if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults.$Auth.$SendCrm) && $RIGHT >= "W" && check_bitrix_sessid())
	{
		...
		if($Auth)
		{
			if($_POST["DOMAIN"] && $_POST["TOKEN"])
			{
				if(count(explode('.', $_POST["DOMAIN"])) > 2)
					$url = str_replace("#DOMAIN#", $_POST["DOMAIN"], \Aspro\Functions\CAsproMaxCRM::FLOWLU_PATH);
				else
					$url = str_replace("#DOMAIN#", $_POST["DOMAIN"], \Aspro\Functions\CAsproMaxCRM::FLOWLU_SHORT_PATH);

				$arPostFields = array(
					"api_key" => $_POST["TOKEN"]
				);
				echo \Aspro\Functions\CAsproMaxCRM::query($url, "/api/v1/module/crm/lead/list", $arPostFields, array());
			}
			else
			{
				$arError = array(
		...
		elseif($SendCrm)
		{
			...
			{
				echo \Aspro\Functions\CAsproMax::sendLeadCrmFromForm($_POST["FORM_ID"], $_POST["RESULT_ID"], 'FLOWLU', $_POST["SITE_ID"]);;
			}
			else
			{
				$arError = array(
			...
			foreach($arTabs as $key => $arTab)
			{
				...
				if($domain && $token)
				{
					if(count(explode('.', $domain)) > 2)
						$url = str_replace("#DOMAIN#", $domain, \Aspro\Functions\CAsproMaxCRM::FLOWLU_PATH);
					else
						$url = str_replace("#DOMAIN#", $domain, \Aspro\Functions\CAsproMaxCRM::FLOWLU_SHORT_PATH);

					$arPostFields = array(
						"api_key" => $token
					);
					$result = json_decode(\Aspro\Functions\CAsproMaxCRM::query($url, "/api/v1/module/crm/lead/list", $arPostFields, array()), true);
			...
			if($arTab["SITE_ID"])
			{
				...
				<?foreach($arTab["ITEMS"] as $groupCode => $arOptions):?>
					<?if($groupCode == "LINK" && !$arOptions["ITEMS"]["ACTIVE_LINK_FLOWLU"]["VALUE"]):?>
						<tr>
							<td colspan="2" style="width:100%;text-align:center;">
								<div class="adm-info-message"><?=Loc::getMessage("ASPRO_MAX_MODULE_INTEGRATION_FLOWLU");?></div>
							</td>
						</tr>
						<?continue;?>
					<?endif;?>

					<tr class="heading"><td colspan="2"><?=Loc::getMessage("ASPRO_MAX_MODULE_".$groupCode."_FLOWLU");?></td></tr>
										...
										<td colspan="2">
											...
											$aSiteTabs = array(
												array(
													...
													"TAB" => Loc::getMessage("ASPRO_MAX_MODULE_FIELDS_FLOWLU"),
													"TITLE" => Loc::getMessage("ASPRO_MAX_MODULE_ALL_FIELDS_FLOWLU"),
													"ICON" => "settings",
													"PAGE_TYPE" => "site_settings",
												),
												array(
													"DIV" => "edit_forms_result",
													"TAB" => Loc::getMessage("ASPRO_MAX_MODULE_RESULTS_FLOWLU"),
													"TITLE" => Loc::getMessage("ASPRO_MAX_MODULE_ALL_RESULTS_FLOWLU"),
													"ICON" => "settings",
													"PAGE_TYPE" => "site_settings",
												),
											);
																				...
																				<tr>
																					<td class="adm-detail-content-cell-l" style="width:50%;">
																						<select name="CRM_FIELD_<?=$optionsSiteID?>[<?=$key;?>][]" class="field_crm" style="width:300px;">
																							<?foreach(\Aspro\Functions\CAsproMaxCRM::$arCrmFileds["FLOWLU"] as $key2 => $text):?>
