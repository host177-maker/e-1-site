	...
	public static function OnBeforeEventAddHandler(&$event, &$lid, &$arFields, &$message_id){
		if(\Bitrix\Main\Loader::includeModule(self::moduleID))
		{
			if($arCurrentRegion = CMaxRegionality::getCurrentRegion())
			{
				...
				$arFields['REGION_ADDRESS'] = is_array($arCurrentRegion['PROPERTY_ADDRESS_VALUE'])?$arCurrentRegion['PROPERTY_ADDRESS_VALUE']['TEXT']:$arCurrentRegion['PROPERTY_ADDRESS_VALUE'];
...
<!DOCTYPE html>', '<!DOCTYPE html>', $content);
	...
	public static function OnBeforeCommentAddHandler(&$arFields){
		...
		if(isset($_REQUEST['user_order_id'])) {
			$arFields['UF_BLOG_ORDER_ID'] = $_REQUEST['user_order_id'];
		}
	...
	public static function OnCommentAddHandler($commentID, &$arFields){
		if($_FILES['comment_images']) {
			...
			if (!empty($_FILES['comment_images1']['name']) || !empty($_FILES['comment_images2']['name'])) {
				$_FILES['comment_images']['name'] = array_merge($_FILES['comment_images']['name'], $_FILES['comment_images1']['name'], $_FILES['comment_images2']['name']);
				$_FILES['comment_images']['tmp_name'] = array_merge($_FILES['comment_images']['tmp_name'], $_FILES['comment_images1']['tmp_name'], $_FILES['comment_images2']['tmp_name']);
				$_FILES['comment_images']['type'] = array_merge($_FILES['comment_images']['type'], $_FILES['comment_images1']['type'], $_FILES['comment_images2']['type']);
				$_FILES['comment_images']['size'] = array_merge($_FILES['comment_images']['size'], $_FILES['comment_images1']['size'], $_FILES['comment_images2']['size']);
			}
			foreach ($_FILES['comment_images']['name'] as $key => $imgName) {
				...
				$file = \CFile::ResizeImageGet($fileId, array('width'=> ($imgSize['width'] ?? 640), 'height'=> ($imgSize['height'] ?? 480)), BX_RESIZE_IMAGE_PROPORTIONAL, true);
				if ($file['src']) {
					$fileIdNew = \CFile::MakeFileArray($file['src']);
					if ($fileIdNew) {
						$fileId = $fileIdNew; //заменяем изображение на сжатое
					}
				}
				if($fileId) {
    ...
    public static function setSeoTitle () {
        global $APPLICATION;
        $arRes = \CIBlockElement::GetList([],
                [
                    'IBLOCK_ID' => \Cosmos\Config::getInstance()->getIblockIdByCode('site_seo'),
                    'NAME' => $APPLICATION->GetCurPage(false),
                ],
        false, false, ['PROPERTY_seo_title_h1']);

        if ($arItem = $arRes->fetch()){
            if  ($arItem['PROPERTY_SEO_TITLE_H1_VALUE']) {
                $APPLICATION->SetTitle(\COrwoSubdomain::makeCurrentCityText($arItem['PROPERTY_SEO_TITLE_H1_VALUE']));
            }
        }
    }
    ...
    public static function getSeoTitle () {
        ...
        if ($arItem = $arRes->fetch()){
            if  ($arItem['PROPERTY_SEO_TITLE_H1_VALUE']) {
                return \COrwoSubdomain::makeCurrentCityText($arItem['PROPERTY_SEO_TITLE_H1_VALUE']);
            }
        }
    }
    ...
    public static function setMeta () {
        ...
        if (CSite::InDir('/catalog/')) {
            ...
            $arrMetaFilter = array();
            $price = array();
            $hlBlock = new Cosmos\HLblock();
            foreach ($GLOBALS['MAX_SMART_FILTER'] as $key => $val) {
                if (is_array($val) && ((count($val) == 2 && is_numeric($val[0]) && is_numeric($val[1])) && !strpos($key, 'PROPERTY')) || strpos($key, 'PRICE')) {
                    $price = $val;
                    $optionsCount++;
                }
                ...
                if ($key == 'OFFERS') {
                    foreach ($val as $itemKey => $itemValue) {
                        $rsProperty = CIBlockPropertyEnum::GetList(
                            array(),
                            array(
                                'IBLOCK_ID' => \Cosmos\Config::getInstance()->getIblockIdByCode('offers'),
                                'ID'        => $itemValue,
                            )
                        );

                        $resCount = 0;
                        while ($enum_fields = $rsProperty->GetNext()) {
                        ...
                        if ($resCount == 0) {
                            $colors = $hlBlock->getElements('TSVETKORPUSA', array('filter' => array('UF_XML_ID' => $itemValue)));
                            if (count($colors) > 0) {
                                foreach ($colors as $color) {
                                    $arrMetaFilter['COLORS'][] = $color['UF_NAME'];
                                }
                            }
                        }
                    ...
                    $property_enums = CIBlockPropertyEnum::GetList(array("DEF" => "DESC", "SORT" => "ASC"), array("IBLOCK_ID" => 48, "ID" => $val));
            ...
            $optionsCount += count($arrMetaFilter);
            if ($optionsCount >= 2) {

                $title = 'Купить ';
                $description = 'У нас можно заказать ';

                if ($arrMetaFilter['TIP_SHKAFA']) {
                    $typeText = '';
                    $typeCount = count($arrMetaFilter['TIP_SHKAFA']['value']) - 1;
                    foreach ($arrMetaFilter['TIP_SHKAFA']['value'] as $key => $propName) {
                        $typeText .= ' ' . $propName;
                        if ($key != $typeCount) {
                            $typeText .= ', ';
                        }
                    }
                    if (strpos($typeText, 'Шкафы') === false){
                        $typeText = 'Шкафы-купе '.$typeText;
                    }
                    $title .= $typeText;
                    $description .= $typeText;
                } elseif ($props['CATALOG_SECTION_NAME']) {
                    if (strpos($name, 'Шкафы') === false){
                        $name = 'Шкафы-купе '.$name;
                    }
                    $title .= $name . ' ';
                    $description .= $name . ' ';
                }

                if (!empty($arrMetaFilter['PHOTO_PRINTING']) && $arrMetaFilter['PHOTO_PRINTING']['value'][0] == 'Да') {
                    $title .= ' с фотопечатью ';
                    $description .= ' с фотопечатью';
                }

                if (!empty($arrMetaFilter['SERIYA_SHKAFA'])) {
                    $seriesText = ' серии ';

                    $seriesCount = count($arrMetaFilter['SERIYA_SHKAFA']['value']) - 1;
                    foreach ($arrMetaFilter['SERIYA_SHKAFA']['value'] as $key => $propName) {
                        $seriesText .= ' ' . $propName;
                        if ($key != $seriesCount) {
                            $seriesText .= ', ';
                        }
                ...
                if ($arrMetaFilter['HEIGHT']) {
                    $heightText = ' высотой ';
                    $heightCount = count($arrMetaFilter['HEIGHT']['value']) - 1;
                    foreach ($arrMetaFilter['HEIGHT']['value'] as $key => $propName) {
                        $heightText .= ' ' . $propName;
                        if ($key != $heightCount) {
                            $heightText .= ', ';
                        }
                    ...
                    $heightText .= ' мм, ';
                    $title .= $heightText;
                    $description .= $heightText;
                }

                if ($arrMetaFilter['WIDTH']) {
                    $widthText = ' шириной ';
                    $widthCount = count($arrMetaFilter['WIDTH']['value']) - 1;
                    foreach ($arrMetaFilter['WIDTH']['value'] as $key => $propName) {
                        $widthText .= ' ' . $propName;
                        if ($key != $widthCount) {
                            $widthText .= ', ';
                        }
                    ...
                    $widthText .= ' мм, ';
                    $title .= $widthText;
                    $description .= $widthText;
                }

                if ($arrMetaFilter['DEPTH']) {
                    $depthText = ' глубиной ';
                    $depthCount = count($arrMetaFilter['DEPTH']['value']) - 1;
                    foreach ($arrMetaFilter['DEPTH']['value'] as $key => $propName) {
                        $depthText .= ' ' . $propName;
                        if ($key != $depthCount) {
                            $depthText .= ', ';
                        }
                ...
                if ($arrMetaFilter['COLORS']) {
                    $colorsText = ' цвет корпуса ';
                    $colorsCount = count($arrMetaFilter['COLORS']) - 1;
                    foreach ($arrMetaFilter['COLORS'] as $key => $colorName) {
                        $colorsText .= ' ' . $colorName;
                        if ($key != $colorsCount) {
                            $colorsText .= ', ';
                        }
                    }
                ...
                if (!empty($price)) {
                    $description .= " по цене от {$price[0]} руб до {$price[1]} руб. ";
                } else {
                    $description .= ' .';
                }

                $description .= ' Фирменные салоны Е1 в 20 регионах России, доставка мебели от 3-х дней, гарантия до 10 лет.';
        ...
        if ($seo_title = $APPLICATION->GetPageProperty('seo_title')){
            $title = $seo_title;
        }

        if ($seo_description = $APPLICATION->GetPageProperty('seo_description')){
        ...
        if ($title){
            $title = \COrwoSubdomain::makeCurrentCityText($title);
            $APPLICATION->SetPageProperty('title', $title);
        }
        if ($description){
            $description = \COrwoSubdomain::makeCurrentCityText($description);
            $APPLICATION->SetPageProperty('description', $description);
        }

        $logoImg = CMax::GetFrontParametrsValues(SITE_ID)["LOGO_IMAGE"];
        $ogTitleHtml = '<meta property="og:title" content="'.$title.'">';
        $ogTypeHtml = '<meta property="og:type" content="website">';
        $ogImgHtml = '<meta property="og:image" content="'.$logoImg.'">';

        $APPLICATION->AddViewContent('customOG', $ogTitleHtml.$ogTypeHtml.$ogImgHtml );
