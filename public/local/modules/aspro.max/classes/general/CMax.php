...
class CMax{
	...
	public static function Start($siteID){
		...
		if(CModule::IncludeModuleEx(self::moduleID) == 1)
		{
			...
			$APPLICATION->SetAdditionalCSS(SITE_TEMPLATE_PATH.'/css/header.min.css', true); // header
	...
	public static function getCurrentBodyClass(){
		...
		$result .= ' fill_bg_n';
		$result .= ' catalog-delayed-btn-'.( isset($arTheme['CATALOG_DELAY']['VALUE']) ? $arTheme['CATALOG_DELAY']['VALUE'] : $arTheme['CATALOG_DELAY'] );		
	...
	public static function IsMainPage(){
		...
		$curPage = $_SERVER['REDIRECT_REGION'] 
				? str_replace('/'. $_SERVER['REDIRECT_REGION'], '', $APPLICATION->GetCurPage(true))
				: $APPLICATION->GetCurPage(true);

		static $result;

		if(!isset($result))
			$result = $curPage === SITE_DIR.'index.php';
	...
	public static function IsCatalogPage($page = ''){
		...
		if(!isset($result))
		{
			...
			$result = substr_count($uri, '/catalog/') > 0;
		}
	...
	public static function showPriceMatrix($arItem = array(), $arParams, $strMeasure = '', $arAddToBasketData = array()){
		...
		{
			ob_start();?>
				<?$bShowPopupPrice = (static::GetFrontParametrValue('SHOW_POPUP_PRICE') == 'Y' && (count($arItem['PRICE_MATRIX']['ROWS']) > 1 || count($arItem['PRICE_MATRIX']['COLS']) > 1));?>
				<?if($bShowPopupPrice):?>
		...
		if($arItem)
		{
			if(isset($arItem['PRICE_MATRIX']) && $arItem['PRICE_MATRIX'])
			{
				ob_start();
					...
					?>
					<?if(static::GetFrontParametrValue('SHOW_POPUP_PRICE') == 'Y' && (count($arItem['PRICE_MATRIX']['ROWS']) > 1 || count($arItem['PRICE_MATRIX']['COLS']) > 1)):?>
	...
	public static function getMenuChildsExt($arParams, &$aMenuLinksExt){
		...
		if($arParams['MENU_PARAMS']['MENU_SHOW_SECTIONS'] == 'Y')
		{
            ...
            foreach($arSections as &$section) {
                if($section['UF_IS_FILE'] && $section['UF_FILE']) {
                    $section['SECTION_PAGE_URL'] = \CFIle::GetPath($section['UF_FILE']);
                    $section['~SECTION_PAGE_URL'] = \CFIle::GetPath($section['UF_FILE']);
                } else if ($section['UF_IS_LINK'] && $section['UF_LINK']) {
                    $section['SECTION_PAGE_URL'] = $section['UF_LINK'];
                    $section['~SECTION_PAGE_URL'] = $section['UF_LINK'];
                }
            }
	...
	public static function GetQuantityArray(
		...
		static $arQuantityOptions, $arQuantityRights, $existTemplates;
		...
		if($arQuantityOptions === NULL){
			$arQuantityOptions = array(
				...
				"EXPRESSION_FOR_NOTEXISTS_ZAKAZ" => Option::get(self::moduleID, "EXPRESSION_FOR_NOTEXISTS_ZAKAZ", GetMessage("EXPRESSION_FOR_NOTEXISTS_ZAKAZ"), $siteId),
        ...
        if ($existTemplates === null) {
            Loader::includeModule('iblock');

            $dbRes = CIBlockElement::GetList(['SORT' => 'ASC', 'ID' => 'ASC'], ['IBLOCK_ID' => Cosmos\Config::getInstance()->getIblockIdByCode('exist_templates'), 'ACTIVE' => 'Y'], false, false, ['ID', 'IBLOCK_ID', 'NAME', 'PROPERTY_FROM', 'PROPERTY_TO']);
            while ($arRes = $dbRes->Fetch()) {
                $existTemplates[] = $arRes;
            }
        }

		$indicators = 0;
		$totalAmount = $totalText = $totalHTML = $totalHTMLs = $totalCountFormatted = '';

        if ($totalCount > 0 || $totalCount == 0) {
            $totalCountFormatted = $totalCount;

            foreach ($existTemplates as $existTemplate) {
                if ($totalCount >= $existTemplate['PROPERTY_FROM_VALUE'] && $totalCount <= $existTemplate['PROPERTY_TO_VALUE']) {
                    $totalCountFormatted = $existTemplate['NAME'];
                    break;
                }
            }
        }
		...
		if($arQuantityRights["SHOW_QUANTITY"]){
			...
			{
				$totalText = "<span class='store_view dotted blue_color'>".(!empty($totalCountFormatted)) ? $totalCountFormatted : $arQuantityOptions["EXPRESSION_FOR_NOTEXISTS_ZAKAZ"].'</span>';
			}
			else
			{
				if($useStoreClick=="Y")
			...
			if($arQuantityRights["SHOW_QUANTITY_COUNT"] && $totalCount > 0)
			{
				...
				{
					if($useStoreClick=="Y")
						$totalText .= (strlen($totalText) ? ": ".$totalCountFormatted : "<span class='store_view dotted'>".$totalCountFormatted.'</span>');
					else
						$totalText .= (strlen($totalText) ? ": ".$totalCountFormatted : $totalCountFormatted);
				}
			}
			...
			$totalHTMLs .= '<span class="icon '.($totalCount > 0 ? 'stock' : ' order').' '.($totalCount == 0 ? ' blue_color' : '').'"></span><span class="value font_sxs '.($totalCount == 0 ? ' blue_color' : '').'">'.$totalText.'</span>';
			$totalHTMLs .='</div>';
		}
			...
			<?if($arRegion):?>
				<?if($arRegion['PROPERTY_EMAIL_VALUE']):?>
					<div <?=($class ? 'class="'.$class.'"' : '')?>>
						...
						<?foreach($arRegion['PROPERTY_EMAIL_VALUE'] as $value):?>
							<?php if($value == 'best@e-1.ru') echo '<span style="color: #999999; font-size: 12px;">Отдел продаж</span>';?>
							<?php if($value == 'service@e-1.ru') echo '<span style="color: #999999; font-size: 12px;">Служба клиентского сервиса</span>';?>
	...
	public static function ShowHeaderPhones($class = '', $bFooter = false){
		...
		<?if($iCountPhones):?>
			...
			$sPhoneFooter = Option::get('e1.site.settings', 'E1_SS_FOOTER_PHONE');
			?>
			<!-- noindex -->
			<div class="phone with_dropdown<?=($class ? ' '.$class : '')?>">
				...
				<?endif;?>
					...
					<?
					//подставляем телефон из настроек сайта, если задан
					if(!empty($sPhoneFooter) && $bFooter) {
						$phoneSettings = $sPhoneFooter;
						$hrefSettings = preg_replace('![^0-9]+!', '', $sPhoneFooter);
						$hrefSettings = 'tel:'. $hrefSettings;?>
						<?=static::showIconSvg("phone", SITE_TEMPLATE_PATH."/images/svg/".($bFooter ? "phone_footer" : "Phone_black").".svg");?><a class="phone-footer" rel="nofollow" href="<?=$hrefSettings?>"><?=$phoneSettings?></a>
					<?
					}
					?>
				<?if($bFooter):?>
				...
				<div class="dropdown <?=($bHaveIcons ? 'with_icons' : '')?>">
					<div class="wrap srollbar-custom scroll-deferred">
						<?for($i = 0; $i < $iCountPhones; ++$i):?>
							...
							?>
							<div class="more_phone"><a rel="nofollow" <?=(strlen($description) ? '' : 'class="no-decript"')?> href="<?=$href?>"><?=$icon?><?=$phone?><?=$description?></a>
							<?
								//подставляем телефон из настроек сайта, если задан
								if(!empty($hrefSettings) && !empty($phoneSettings) && $bFooter) {
									?>
									<a rel="nofollow" <?=(strlen($description) ? '' : 'class="no-decript"')?> href="<?=$hrefSettings?>"><?=$icon?><?=$phoneSettings?><?=$description?></a>
									<?
								}
								?>
							</div>
		...
		if(count($basketItemsById) > 0){
			...
			if (!empty($arParentId)) {
				foreach ($basket as $item)
				{
					...
					{
						//удаление модуля при переключении
						 if ((string)$property->getField('NAME') == 'OFFER_ID' && (string)$property->getField('CODE') == 'OFFER_ID' && (string)$arParentId == (string)$property->getField('VALUE') && !is_array($arParentId) || is_array($arParentId) && (string)$property->getField('NAME') == 'OFFER_ID' && (string)$property->getField('CODE') == 'OFFER_ID' && $arParentId[0] == (string)$property->getField('VALUE')) {
							try
							{
