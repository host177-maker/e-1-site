						...
						if($this->arResult["Perm"] >= BLOG_PERMS_PREMODERATE)
						{
							if(check_bitrix_sessid())
							{
								...
								if($_POST["act"] != "edit")
								{
									...
									if($_POST["user_order_id"] == '')
										$strErrorMessage .= GetMessage("B_B_PC_NO_ORDER_ID")."<br />";
									if($_POST["user_order_email"] == '')
										$strErrorMessage .= "Не введен email<br />";
									if($_POST["user_order_phone"] == '')
										$strErrorMessage .="Не введен номер телефона<br />";
...
//				for only visible comments (only current page) use conversion and geneerate additional params
	...
	protected function createCommentsList()
	{
		...
		if ($this->arParams["CACHE_TIME"] > 0 && $cache->InitCache($this->arParams["CACHE_TIME"], $cacheId, $cachePath))
		{
			...
			(new CBitrixComponentTemplate)->ApplyCachedData($vars["templateCachedData"]);
			$cache->Output();
		}
		else
		{
