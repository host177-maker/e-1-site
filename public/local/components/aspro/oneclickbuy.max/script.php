...
else
{
	...
	if(!function_exists('getJsonCustom')) {
		function getJsonCustom($message, $res = 'N', $error = '', $ext = false, $getblue_data = '', $arSlonItems = [], $orderPrice = '', $arOrder = '', $arPostData = ''){
			$GLOBALS['APPLICATION']->RestartBuffer();
			...
			$result = array(
				...
				'arSlonItems' => $arSlonItems,
				'productSumm' => $orderPrice,
				'orderId' => $message . '_' .$arPostData['ONE_CLICK_BUY']['PHONE'] . '_' . time(),
			);
			if(\Bitrix\Main\Config\Option::get('aspro.max', 'ONE_CLICK_BUY_CAPTCHA', 'N') == 'Y'){
				if(!is_array($ext)){
					$ext = array();
	...
	$obBasketItems = $order->getBasket();
		...
		while($arBasketItem = $resBasketItems->Fetch()){
			...
			if($arBasketItem['CAN_BUY'] === 'Y'){
				...
				$resultSlonItems['arrSlonItems'][] = [
					'id' => $arBasketItem['ID'],
					'quantity' => $arBasketItem['QUANTITY'],
					'price' => $arBasketItem['PRICE']
				];
				$productsForGetblue .= ','.$arBasketItem["PRODUCT_ID"];
			}
		}
	...
	echo getJsonCustom($orderID, 'Y', '', false, $productsForGetblue, $resultSlonItems, $orderPrice, $arOrder, $_POST);
	die();
}