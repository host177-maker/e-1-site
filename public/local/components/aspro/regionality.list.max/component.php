...
global $arTheme, $APPLICATION, $_COOKIE;
?>
<?if(\Bitrix\Main\Loader::includeModule('aspro.max'))
{
	...
	if(!$arResult['POPUP']  || ($arResult['POPUP'] && \Bitrix\Main\Config\Option::get('aspro.max', 'REGIONALITY_SEARCH_ROW', 'N') != 'Y'))
	{
		$arResult['REGIONS'] = CMaxRegionality::getRegions();
		$arResult['CURRENT_REGION'] = CMaxRegionality::getCurrentRegion();
		$arResult['REAL_REGION'] = CMaxRegionality::getRealRegionByIP();
		...
		//здесь нужно отображать подтверждение города в первый раз, почему то было отключено, переделал
		if ($arParams["SHOW_MODAL"] && $arParams['TEMPLATE'] === 'regions_custom' && empty($request->getCookie("popupCity"))){
			$arResult['SHOW_REGION_CONFIRM'] = true;
			$cookie = new \Bitrix\Main\Web\Cookie("popupCity", 'true', time()+86400*30);
	...
	if($arResult['REGIONS'])
	{
		...
		if($arResult['POPUP'])
		{
			...
			if($arSectionsID)
			{
				$arSections = CMaxCache::CIBlockSection_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('GROUP' => 'ID', 'TAG' => CMaxCache::GetIBlockCacheTag(CMaxCache::$arIBlocks[SITE_ID]['aspro_max_regionality']['aspro_max_regions'][0]))), array('GLOBAL_ACTIVE' => 'Y', 'ID' => $arSectionsID, 'IBLOCK_ID' => CMaxCache::$arIBlocks[SITE_ID]['aspro_max_regionality']['aspro_max_regions'][0]), false, array('ID', 'IBLOCK_ID', 'NAME', 'LEFT_MARGIN', 'RIGHT_MARGIN', 'DEPTH_LEVEL'));
				if($arSections)
				{
					$arSections2 = array();
					foreach($arSections as $key => $arSection)
					{
						...
						if($arSection['DEPTH_LEVEL'] > 1)
						{
							...
							$arTmpSection = CMaxCache::CIBlockSection_GetList(array('CACHE' => array('MULTI' => 'N', 'TAG' => CMaxCache::GetIBlockCacheTag(CMaxCache::$arIBlocks[SITE_ID]['aspro_max_regionality']['aspro_max_regions'][0]))), array('GLOBAL_ACTIVE' => 'Y', '<=LEFT_BORDER' => $arSection['LEFT_MARGIN'], '>=RIGHT_BORDER' => $arSection['RIGHT_MARGIN'], 'DEPTH_LEVEL' => 1, 'IBLOCK_ID' => CMaxCache::$arIBlocks[SITE_ID]['aspro_max_regionality']['aspro_max_regions'][0]), false, array('ID', 'IBLOCK_ID', 'NAME', 'SORT'));
	...
	elseif($arResult['POPUP'] && \Bitrix\Main\Config\Option::get('aspro.max', 'REGIONALITY_SEARCH_ROW', 'N') == 'Y')
	{
		$APPLICATION->AddHeadScript(SITE_TEMPLATE_PATH.'/js/jquery-ui.min.js');
		$this->IncludeComponentTemplate();
	}
