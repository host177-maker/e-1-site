...
else
{
	...
	if(!function_exists('getJsonCustom')) {
		function getJsonCustom($message, $res = 'N', $prod_summ = 0, $fb_data = array(), $items_ids = array(), $vk_data = array(), $getblue_data = '', $arrSlonItems = '', $arOrder = '', $arPostData = '', $error = '', $ext = false){
			$GLOBALS['APPLICATION']->RestartBuffer();

			$arOrder['prod_summ'] = $prod_summ;
			$arComagicSendData = COrwoFunctions::GetComagicSendData($arOrder, $arPostData, 'oneclickbuy');

			$result = array(
				'result' => $res === 'Y' ? 'Y' : 'N',
				'message' => $GLOBALS['APPLICATION']->ConvertCharset($message, SITE_CHARSET, 'utf-8'),
				'productSumm' => $prod_summ,
				'fbData' => $fb_data,
				'productIds' => $items_ids,
				'vkData' => $vk_data,
				'getblueIds' => $getblue_data,
				'arrSlonItems' => $arrSlonItems,
				//'orderId' => $message,
				'orderId' => $message . '_' .$arComagicSendData['phone'] . '_' . time(),
				'arComagicSendData' => CUtil::PhpToJSObject($arComagicSendData)
			);
	...
	if(class_exists('\Bitrix\Sale\Delivery\Services\Table')){
		$deliveryCode = intval($deliveryId) > 0 ? \Bitrix\Sale\Delivery\Services\Table::getCodeById($deliveryId) : "";
	}
	...
	if($bAllBasketBuy){
		...
		if($arBasketItems){
			...
			$arOrderDat = CSaleOrder::DoCalculateOrder($SITE_ID, $registeredUserID, $arBasketItems, $newOrder['PERSON_TYPE_ID'], array(), $deliveryCode, $newOrder['PAY_SYSTEM_ID'], array(), $arErrors, $arWarnings);
	...
	if($orderID){
		...
		//делаем здесь пересчет для оптимы и корректируем цену для купить в 1 клик
	$order = \Bitrix\Sale\Order::load($orderID);
	$obBasketItems = $order->getBasket();
        ...
        if(!empty(\Cosmos\Config::getInstance()->getParam("LOGGING")['VUE_ORDER'])) {
            file_put_contents(
                $_SERVER['DOCUMENT_ROOT'] . "/../logs/BUY_1KLICK.log",
                "\n\n" . date('Y-m-d H:i:s') . "\n " . print_r([time(), $arFields['ORDER_PRICE'], $_REQUEST,$result  ], true) . "\n\n",
                FILE_APPEND
            );
		...
		while($arBasketItem = $resBasketItems->Fetch()){
			...
			if($arBasketItem['CAN_BUY'] === 'Y'){
				...
				$orderList .= 'product_id='.$arBasketItem["PRODUCT_ID"].' quantity='.$arBasketItem['QUANTITY']. ' ';
				$orderList .= GetMessage('ITEM_NAME') . $arBasketItem['NAME']
		...
		if($_POST['PAGE_URL']){
			$arMessageFields["PAGE_URL"]=$_POST['PAGE_URL'];
		}
		...
		while ($arItem = $dbBasketItems->Fetch()) {
			if($arItem["BASE_PRICE"] != 0) {
				...
				if (is_array($mxResult)) {
					$productInfo = array(
						//'id' => $mxResult["ID"],//заменить на $arItem["PRODUCT_ID"], если в фиде будут передаваться id торговых предложений
					...
					//$arrProductIds[] = $mxResult["ID"];
					$arrProductIds[] = $arItem["PRODUCT_ID"];
				} else {
					$productInfo = array(
						'id' => $arItem["PRODUCT_ID"],
						'quantity' => $arItem["QUANTITY"]
				...
				$productObj = json_encode_custom($productInfo);
				$productsForFb[] = $productObj;
				$productObjVk = json_encode_custom($productInfoVk);
				$productsForVk[] = $productObjVk;
				//$productsForGetblue .= ','.$mxResult["ID"];//заменить на $arItem["PRODUCT_ID"], если в фиде будут передаваться id торговых предложений
				$productsForGetblue .= ','.$arItem["PRODUCT_ID"];
				$arrSlonItems[] = json_encode_custom($productSlonInfo);
			}
		}
	}
	if($productsForGetblue != '') {
		$productsForGetblue = substr($productsForGetblue, 1);
	}
	...
	echo getJsonCustom($orderID, 'Y', $productPriceSumm, $productsForFb, $arrProductIds, $productsForVk, $productsForGetblue, $arrSlonItems, $arOrder, $_POST);
	die();
}