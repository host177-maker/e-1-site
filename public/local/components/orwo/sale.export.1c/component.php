...
else
{
	...
	elseif($_GET["mode"]=="success")
	{
		...
		{
			$_SESSION["BX_CML2_EXPORT"][COrwo1CSaleExport::getOrderPrefix()] = 0;
	...
	elseif($_GET["mode"] == "file" && $_SESSION["BX_CML2_EXPORT"]["version"] == '')// old version
	{
		...
		{
			$position = false;
			$loader = new COrwo1CSaleOrderLoader;
	...
	elseif($_GET["mode"] == "import" && $_SESSION["BX_CML2_EXPORT"]["zip"] && mb_strlen($_SESSION["BX_CML2_EXPORT"]["zip"]) > 1)
	{
		...
		$result = COrwo1CSaleExport::UnZip($_SESSION["BX_CML2_EXPORT"]["zip"], $_SESSION["BX_CML2_EXPORT"]["last_zip_entry"]);
		if($result===false)
		{
