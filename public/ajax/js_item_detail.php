...
<?if($arPost["PARAMS"]):?>
	...
	else
	{
		...
		if(!empty($arPost['ID'])) {
			$rsItems = CIBlockElement::GetList(array(), array("ID" => $arPost['ID']), false, false,
			 [
				'ID',
				'PROPERTY_WIDTH',
				'PROPERTY_DEPTH',
				'PROPERTY_HEIGHT',
				'PROPERTY_HOUSING_LAYOUT', 
		]);
			while($arItemTmp = $rsItems->Fetch())
			{
				$element = $arItemTmp;
		...
		$arFilter = array_merge($arFilter, [
			'PROPERTY_HOUSING_LAYOUT' => $element['PROPERTY_HOUSING_LAYOUT_VALUE'],
			// 'PROPERTY_COLOR_REF' => $element['PROPERTY_COLOR_REF_VALUE'],
			'PROPERTY_WIDTH' => $element['PROPERTY_WIDTH_ENUM_ID'],
			// 'PROPERTY_DEPTH' => $element['PROPERTY_DEPTH_VALUE_ID'],
			// 'PROPERTY_HEIGHT' => $element['PROPERTY_HEIGHT_VALUE_ID']
		]);
		...
		foreach($arResult["ITEMS"] as $key => $arOffer)
		{
            ...
            if($arOffer['PROPERTIES']['CML2_LINK']['VALUE'] > 0) {
                if(empty($arElement)) {
                    $arItemRes = CIBlockElement::GetByID($arOffer['PROPERTIES']['CML2_LINK']['VALUE']);
                    if($arItem = $arItemRes->GetNextElement()) {
                        $props = $arItem->GetProperties();
                        $arElement['PROPERTIES'] = $props;
                    }
                }

                COrwoFunctions::ChangeCatalogItemPrice( $arElement, $arOffer, $arPost['PARAMS']);
	...
	if($arItems)
	{
		foreach($arItems["ITEMS"] as $key => $arItem)
		{
			...
			$arPost["PARAMS"]["SHOW_DISCOUNT_PERCENT"] = "N";

			$arItems["ITEMS"][$key]["PRICES_HTML"] = \Aspro\Functions\CAsproMaxItem::showItemPrices($arPost["PARAMS"], $arItem["PRICES"], $arItem['MEASURE'], $arItem["MIN_PRICE"]["ID"], "N", false, true);
			$arItems["ITEMS"][$key]["PRICES_HTML"] = \Aspro\Functions\CAsproMaxItem::showItemPricesSectionForFilter((int)$arItem['ID'], $arPost["PARAMS"], $arItem["PRICES"], '', $arItem["MIN_PRICE"]["ID"], "N", false, true);
            //$arItems["ITEMS"][$key]["PRICES_HTML"] = \Aspro\Functions\CAsproMaxSku::showItemPrices($arPost["PARAMS"], $arItem, $item_id, $arItem["MIN_PRICE"]["ID"], array(), ($arPost["PARAMS"]["SHOW_DISCOUNT_PERCENT_NUMBER"] == "Y" ? "N" : "Y"), true);
	...
	<script>
		...
		setPrice = function(obPrices, measure, obj)
		{
			...
			if (wrapper.find('.cost.prices').length){
				...
				if(typeof(obPrices) == 'object')
				{
					...
					wrapper.find('.cost .js_price_wrapper').html(obj.PRICES_HTML + '<yandex-pay-badge merchant-id="5ef67561-d34e-45e2-bb25-8320a4653548" type="cashback" amount="'+ obj.MIN_PRICE.VALUE_VAT+ '" size="m" theme="light" align="left" color="primary" />');
		...
		ChangeInfo = function()
		{
			...
			if(-1 < index)
			{
				...
				{
					wrapper.find('.item-title > a').attr('href', obOffers[index].URL);
					wrapper.find('.thumb').attr('href', obOffers[index].URL);
				}
