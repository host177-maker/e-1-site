...
if (isset($_REQUEST["type"]) && $_REQUEST["type"] == "multiple") {
	...
	if ($request["action"] && $request["items"]) {
		...
		$rsItems = CSaleBasket::GetList(
			...
			false,
			false,
			array("ID", "PRODUCT_ID", "DELAY", "QUANTITY")
		);
	...
	if (!empty($_REQUEST["add_item"])) {
		if ($_REQUEST["add_item"] == "Y") {
			...
			if (!empty($dbBasketItems) && $dbBasketItems["DELAY"] == "Y") {
				...
				if ($successfulAdd) {
					...
					if ($restricted = \BuyRestriction::isRestrictedGoods($_REQUEST["item"])) {
						$product_properties[] = [
							'NAME' => \BuyRestriction::RESTRICTION_CART_PROPERTY,
							'CODE' => \BuyRestriction::RESTRICTION_CART_PROPERTY,
							'VALUE' => serialize($restricted)
						];
					}
					...
					if (isset($_REQUEST["prop"]['ASPRO_BUY_PRODUCT_ID']) && $_REQUEST["prop"]['ASPRO_BUY_PRODUCT_ID'] > 0) {
						...
						$elems = CSaleBasket::GetList([], ["FUSER_ID" => CSaleBasket::GetBasketUserID()])->Fetch();
					...
					/*add_services*/
					if (!empty($_REQUEST["services"]) && $successfulAdd) //buy items
					{
						...
						foreach ($_REQUEST["services"] as $arItem) {
							...
							{
								if (intval($arItem['iblock_id']) === 23 && !empty($arItem['custom_price'])) {
									$arRewriteFields['CURRENCY'] = Bitrix\Currency\CurrencyManager::getBaseCurrency();
									$arRewriteFields['PRICE'] = $arItem['custom_price'];
									$arRewriteFields['CUSTOM_PRICE'] = 'Y';
								} else {
									unset($arRewriteFields['CUSTOM_PRICE']);
								}
								
								$arRewriteFields['RECOMMENDATION'] = $_REQUEST["prop"]['ASPRO_BUY_PRODUCT_ID'];
					...
					/*add_assoc*/
					if (!empty($_REQUEST["assocNeedAdd"])) //buy items
					{
						foreach ($_REQUEST["assocNeedAdd"] as $arItem) {
							if ($arItem["id"]) // not empty id
							{
								$arRewriteFields = array();
								$product_properties_assoc = array();
								if (!Add2BasketByProductID($arItem["id"], $arItem["quantity"], $arRewriteFields, $product_properties_assoc)) {
									if ($ex = $APPLICATION->GetException()) {
										$strErrorExt .= '<br>' . $ex->GetString();
									}
		...
		if ($_REQUEST["subscribe_item"] == "Y") {
			if (class_exists('\Bitrix\Catalog\Product\SubscribeManager')) {
				...
				$dbBasketItems = CSaleBasket::GetList(
					...
					false,
					false,
					array("ID", "PRODUCT_ID", "SUBSCRIBE", "CAN_BUY")
				)->Fetch();
	...
	} elseif (!empty($_REQUEST["delete_assembly"])) {
		$deleteId = htmlspecialcharsEx($_REQUEST["delete_assembly"]);
		$basket = Sale\Basket::loadItemsForFUser(\Bitrix\Sale\Fuser::getId(), \Bitrix\Main\Context::getCurrent()->getSite());

		foreach ($basket as $item) {
			if ($item->getField('PRODUCT_ID') == $deleteId) {
				try {
	...
	} elseif (!empty($_REQUEST["add_service"])) {
		try {
			$parentId = htmlspecialcharsEx($_REQUEST["parentId"]);
			$parentPrice = htmlspecialcharsEx($_REQUEST["parentPrice"]);
			$priceModule = htmlspecialcharsEx($_REQUEST["priceModule"]);
			$productId = $_REQUEST["add_service"];

			if ($parentId && ($parentPrice || $priceModule)) {
				$product_properties_services = $arRewriteFields = [];
				$product_properties_services = CIBlockPriceTools::CheckProductProperties(
					23,
					$productId,
					array("BUY_PRODUCT_PROP"),
					array("BUY_PRODUCT_PROP" => $parentId),
					'Y'
				);

				$product_properties_services = is_array($product_properties_services) ? $product_properties_services : array();
				$product_properties_services[] = array("NAME" => 'link_id', "CODE" => 'ASPRO_BUY_PRODUCT_ID', 'VALUE' => $parentId);

				//need for sort in admin
				$dbBasketItemParent = CSaleBasket::GetList(
					array("NAME" => "ASC", "ID" => "ASC"),
					array("PRODUCT_ID" => $parentId, "FUSER_ID" => CSaleBasket::GetBasketUserID(), "LID" => SITE_ID, "ORDER_ID" => "NULL"),
					false,
					false,
					array("ID", "DELAY", "SORT")
				)->Fetch();
				...
				if ($parentPrice) {
					$arRewriteFields['PRICE'] = ((int) $parentPrice / 10) <= 1499 ? 1499 : (int) $parentPrice / 10;
				} else {
					$arRewriteFields['PRICE'] = (int) $priceModule <= 1499 ? 1499 : (int) $priceModule;
				}
				$arRewriteFields['CURRENCY'] = Bitrix\Currency\CurrencyManager::getBaseCurrency();
				$arRewriteFields['CUSTOM_PRICE'] = 'Y';
				$arRewriteFields['RECOMMENDATION'] = $parentId;

				if (!Add2BasketByProductID($productId, 1, $arRewriteFields, $product_properties_services)) {
					if ($ex = $APPLICATION->GetException()) {
						$strErrorExt .= '<br>' . $ex->GetString();
					}
			...
			file_put_contents(
				$_SERVER['DOCUMENT_ROOT'] . "/add_service_2_basket.log",
				"\n\n" . date('Y-m-d H:i:s') . "\n " . print_r([$th->getMessage()], true) . "\n\n",
				FILE_APPEND
			);
		}
	} elseif (!empty($_REQUEST["update_service"])) {
		$dbBasketItems = CSaleBasket::GetList(
			array("NAME" => "ASC", "ID" => "ASC"),
			array(
				"RECOMMENDATION" => $_REQUEST["update_service"],
		...
		while ($arItem = $dbBasketItems->Fetch()) {
			$updateId = htmlspecialcharsEx($arItem["ID"]);
