...
async function sendToWebhook(data: InstallmentRequest): Promise<{ success: boolean; response?: WebhookResponse; error?: string }> {
  try {
...
async function sendEmailFallback(data: InstallmentRequest, webhookError: string): Promise<boolean> {
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
      <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
          ...
          <tr>
            <td style="padding: 10px; border-bottom: 1px solid #eee; font-weight: bold;">Тип заявки:</td>
            <td style="padding: 10px; border-bottom: 1px solid #eee;">Запрос рассрочки</td>
          </tr>
          <tr>
...
Причина: ${webhookError}
  ...
  try {
    await transporter.sendMail({
      ...
      to: saleMail,
      subject: `Заявка на рассрочку от ${data.name} - ${data.phone}`,
      text: emailText,
      html: emailHtml,
    });
    return true;
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Request installment error:', error);
    return NextResponse.json(
      {
        success: false,
