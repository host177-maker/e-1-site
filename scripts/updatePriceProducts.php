...
function recursive()
{
    ...
    if (file_exists($strPathDir . '/updatePriceProducts.log')) {
        $count = file_get_contents($strPathDir . '/updatePriceProducts.log');
        $count = intval($count);
    } else {
    ...
    while ($arElement = $resDb->Fetch()) {
        ...
        foreach ($offers as $offer) {
            ...
            if ($offer["PROPERTIES"]["BASIC_CONFIGURATION"]["VALUE"] == 'Да') {
                $arResult['DISCOUNT'] = $arGetPrice;
                $arResult['OFFER'] = $rsOffer;
            }
        }
        //$arPriceProduct = CCatalogProduct::GetOptimalPrice($arElement["ID"], 1, $USER->GetUserGroupArray(), 'N');

        if (isset($arResult['OFFER']) && !empty($arResult['OFFER'])) {
            $rsOffer['PRICES'] = $arResult['OFFER']['PRICES'];
        }
        $arPropertyValuesPrice = [];
        foreach ($rsOffer['PRICES'] as $codePrice => $arPrice) {
            $arPropertyValuesPrice["SALE_PRICE_PRODUCT"][] = ['VALUE' => $arPrice["DISCOUNT_VALUE"], 'DESCRIPTION' => $codePrice];
            $arPropertyValuesPrice["PRICE_PRODUCT"][] = ['VALUE' => $arPrice["VALUE"], 'DESCRIPTION' => $codePrice];
        }

        try {
            //обнуляем
            $arProperty["SALE_PRICE_PRODUCT"][] = ["VALUE" => ""];
            $arProperty["PRICE_PRODUCT"][] = ["VALUE" => ""];
            \CIBlockElement::SetPropertyValuesEx($intIdElement, $aCosmosConfigIblock["1c_catalog"]["ID"], $arProperty);
            //обновляем
    ...
    unset($arPropertyValuesPrice, $arElement, $arResult, $offers, $offer, $rsElement, $rsOffer, $arGetPrice, $arPrice, $dbElement, $obElement, $resDb);

    $count = $intIdElement;
    //пишем во временный файл количество, а в начале считываем, и так проходим по всем элементам с шагом 50 элементов
    file_put_contents($strPathDir . '/updatePriceProducts.log', $count);
...
while(1) {
    ...
    if ($count < (int) $resElementEnd) {
        ...
        unlink($strPathDir . '/updatePriceProducts.log');
        break;
    }
}
